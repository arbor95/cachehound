...
/**
 * This class allows handling of a user click on a cache in the radar panel.
 * 
 * @see RadarPanel
 */
public class myInteractivePanel extends InteractivePanel {
	...
	public void imageClicked(AniImage which, Point pos) {
		...
		// If the pen rested more than 500 msec, we only display the info and
	...
	public void onPenEvent(PenEvent ev) {
		...
		if (ev.type == PenEvent.PEN_UP) {
			clearInfo();
			// The next line is needed due to a bug in EWE (it does not call
			// penReleased)
			if (isDragging)
	...
	public boolean imageBeginDragged(AniImage which, Point pos) {
		...
		if (which instanceof RadarPanelImage) {
			...
			int tw, th;
			Image img = new Image(tw = fm.getTextWidth(wayPoint + 15), th = fm
					.getHeight() > 15 ? fm.getHeight() : 15);
			Graphics g = new Graphics(img);
			g.setFont(font);
			g.setColor(Color.White);
	...
	public boolean imageDragged(ImageDragContext drag, Point pos) {
		if (drag.image != null) {
			/*
			 * Point p = Gui.getPosInParent(this,getWindow()); p.x +=
			 * pos.x-origin.x; p.y += pos.y-origin.y; Control c =
			 * getWindow().findChild(p.x,p.y);
			 */
