...
public class RadarPanel extends CellPanel {
	...
	/**
	 * Constructor for the radar panel. Loads images, sets up the interactive
	 * panel and "navigation" buttons.
	 */
	public RadarPanel() {
		this.addLast(iActP = new myInteractivePanel(), CellConstants.STRETCH,
				CellConstants.FILL);
		CellPanel cp = new CellPanel();
		cp.addNext(btMinus, CellConstants.HSTRETCH,
				(CellConstants.FILL | CellConstants.WEST));
		cp.addNext(btToggle, CellConstants.HSTRETCH, CellConstants.FILL);
		cp.addLast(btPlus, CellConstants.HSTRETCH,
				(CellConstants.FILL | CellConstants.EAST));
	 ...
	 * database. It also calculates the maximum size available for drawing the
	 * radar.
	 */
	...
	/**
	 * Public method to draw the different caches and the radar background
	 */
	public void drawThePanel() {
		...
		iActP.repaintNow();
		if (reCenterImage) {
			// Hack to scroll to left origin for a defined position for
			// subsequent
			// scroll which centers the image
			iActP.scroll(-1000, -1000);
			Dimension dispSize = getDisplayedSize(null);
			iActP.scroll((width - dispSize.width) / 2, (height
					- dispSize.height + btMinus.getSize(null).height) / 2);
			reCenterImage = false;
		}
	}
	...
	private void drawCaches() {
		...
		for (int i = cacheDB.size() - 1; i >= 0; i--) {
			...
			if (holder.isVisible() && holder.getPos().isValid()) {
				degrees = holder.getDegrees() * pi180;
				drX = new Float(holder.getKilom() / scale
						* java.lang.Math.sin(degrees)).intValue();
				drY = -new Float(holder.getKilom() / scale
						* java.lang.Math.cos(degrees)).intValue();
				if (centerX + drX >= 0 && centerY + drY >= 0
						&& centerX + drX <= width && centerY + drY <= height) {
					if (toggleMod > 0) {
						String s;
						if (toggleMod == 1)
						...
						if (s.length() > 0) {
							int tw;
							Image img = new Image(tw = fm.getTextWidth(s), fm
									.getHeight());
	...
	/**
	 * Private method to draw the black background and green radar. Also
	 * calculates some other parameters. Always call this before calling
	 * drawCaches().
	 */
	private void drawBackground() {
		...
		for (int i = 1; i <= steps; i++) {
			...
			g.drawEllipse(centerX - radius / 2, centerY - radius / 2, radius,
					radius);
		}
		g.drawLine(centerX, 0, centerX, height);
		g.drawLine(0, centerY, width, centerY);

		// Show 1 KM radius only if we have zoomed in (useful for cities with
		// high density of caches)
		if (scaleKm <= 20) {
