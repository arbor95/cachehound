...
/**
 * This class displays a form to show and edit notes for a cache. Class ID=
 */
public class NotesScreen extends Form {
	...
	public NotesScreen(CacheHolderDetail ch) {
		...
		this.title = "Notes";
		setPreferredSize(Global.getPref().myAppWidth,
				Global.getPref().myAppHeight);
		this.resizeOnSIP = true;
		chD = ch;
		wayNotes.setText(chD.getCacheNotes());
		addLast(sbp.setTag(CellConstants.SPAN, new Dimension(3, 1)),
				CellConstants.STRETCH,
				(CellConstants.FILL | CellConstants.WEST));
		titleControls = new CellPanel();
		titleControls.addNext(addDateTime, CellConstants.HSTRETCH,
				CellConstants.HFILL);
		titleControls.addNext(cancelBtn, CellConstants.HSTRETCH,
				CellConstants.HFILL);
		titleControls.addLast(btSave, CellConstants.HSTRETCH,
				CellConstants.HFILL);
	}
		...
		if (ev instanceof ControlEvent && ev.type == ControlEvent.PRESSED) {
			...
			if (ev.target == cancelBtn) {
				if ((!chD.getCacheNotes().equals(wayNotes.getText()))) {
					if ((new MessageBox(
							"Warning",
							"You will loose any changes made to the notes. Do you want to continue?",
			...
			if (ev.target == titleOK) {
				if ((!chD.getCacheNotes().equals(wayNotes.getText()))) {
					if ((new MessageBox("Warning",
							"Save changes made to the notes?", FormBase.YESB
									| FormBase.NOB)).execute() == FormBase.IDYES) {
						chD.setCacheNotes(wayNotes.getText());
						chD.getParent().save();
					}
				}
