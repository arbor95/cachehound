...
public class TravelbugPickup {
	...
	public static Travelbug pickupTravelbug(TravelbugList tbl) {
		Travelbug tb = null;
		TravelbugScreen tbs = new TravelbugScreen(tbl, MyLocale.getMsg(6016,
				"Pick up travelbug"), true);
		...
		if (tbs.selectedItem >= 0) { // Was a TB selected ?
			...
			if (tbs.selectedItem == tbl.size()) {
				InputBox ibox = new InputBox(MyLocale.getMsg(6018,
						"Travelbug name"));
			...
			InputBox ibox = new InputBox(MyLocale.getMsg(6019,
					"Tracking number"));
