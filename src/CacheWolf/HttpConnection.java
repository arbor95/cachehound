...
/**
 * Use this class to create an HttpConnection with a Web Server and to read in
 * the data for the connection.
 * <p>
 * This a a modified version of XXX. This version automatically makes use of a
 * proxy server if once for all proxy is set. To use this do the following:
 * <ol>
 * <li>Create an HttpConnection object with a URL or specify the host, port and
 * document to get.
 * <li>Change any of the HttpConnection parameters (including documentIsEncoded
 * if your document is URL encoded - i.e. it has '?' type data within it) and
 * set requestor properties as needed.
 * <li>Call connectAsync() or connect() to make a Socket that is connected with
 * the server. These methods will also send the Http request (e.g. GET or POST),
 * send the requestor parameters and then read in and parse the server response
 * and server parameters. When these methods are complete the next data to be
 * read in from the Socket will be the actual data bytes for the http tranfer.
 * <li>Call redirectTo() to see if the request resulted in a redirection
...
public class HttpConnection {
	 ...
	 * valid after a connection has been made since it is sent by the server to
	 * the requestor. One properties that will always be in this list will be
	...
	/**
	 * Set these when the class is instantiated the first time. afterwards you
	 * don't need to set proxy parameters anymore
	 */

	private static String proxy = Global.getPref().myproxy;
	private static int proxyPort = Common
			.parseInt(Global.getPref().myproxyport);
	...
	/**
	 * Get a new HttpConnection whose parameters are copied from this
	 * HttpConnection but which directs its request to a different host/document
	 * as directed by a redirect response from an initial request.
	 * <p>
	 * Note that if this is a POST request and if the post-data source is an
	 * InputStream or Stream, then you will need to call setPostData() again to
	 * set up the post data for the new connection. Otherwise, the post data
	 * will be copied to this device. Call hasPostData() to determine if the
	 * post data was copied across successfully.
	 * 
	 * @param redirectTo
	 *            the URL that the server instructed the client to redirect to.
	...
	public HttpConnection getRedirectedConnection(String redirectTo) {
		...
		c.command = command;
		c.textCodec = textCodec == null ? null : (TextCodec) textCodec
				.getCopy();
	...
	/**
	 * Returns the requestor properties. These are the property commands sent to
	 * the server when the connection is made. You can add directly to this OR
	 * you can call setRequestorProperty() or addRequestorProperty();
	 */
	// ===================================================================
	...
	/**
	 * Set the data to post out as either a Stream, InputStream,byte[],ByteArray
	 * or String. If the data is a Stream or InputStream then you must also call
	 * setPostDataLength() which in turn sets the "Content-Length" property of
	 * the requestor properties - otherwise if "Content-Length" is not already
	 * set it will be set to the length of the byte[] or ByteArray.
	 * 
	 * @param data
	 *            the data to post either as a Stream, InputStream, byte[] or
	 *            ByteArray
	 */
	// ===================================================================
	...
	// ===================================================================
	{
		if (data instanceof Stream)
			...
			getRequestorProperties().defaultTo("Content-Length",
					Convert.toString(((ByteArray) data).length));
		} else if (data instanceof byte[]) {
			originalPostData = data;
			bytesToPost = new MemoryFile(new ByteArray((byte[]) data));
			getRequestorProperties().defaultTo("Content-Length",
					Convert.toString(((byte[]) data).length));
			...
			if (td == null)
				td = new AsciiCodec();
			try {
				ByteArray got = td.encodeText(Vm.getStringChars(s), 0, s
						.length(), true, null);
	...
	/**
	 * This sets the "Content-Length" requestor property to be the specified
	 * length.
	 * 
	 * @param length
	 *            the number of bytes to be posted.
	 */
	// ===================================================================
	...
	// ===================================================================
	{
		getRequestorProperties()
				.set("Content-Length", Convert.toString(length));
	}

	/**
	 * Set an exclusive requestor property. These are sent to the web server
	 * after the initial request line.
	 * 
	 * @param name
	...
	/**
	 * Add a non-exclusive requestor property. These are sent to the web server
	 * after the initial request line.
	 * 
	 * @param name
	 *            The name of the property.
	...
	/**
	 * Set the default value of a requestor property. If the value is already
	 * set this will have no effect. Otherwise the value will be set to
	 * defaultValue.
	 * 
	 * @param name
	 *            the name of the property.
	 ...
	 * this method and the HttpConnection protocol will be done over this
	 * Socket.
	 * 
	 * @param sock
	...
	/**
	 * Create a new HttpConnection to the specified host and port to fetch the
	 * specified document.
	 * 
	 * @param host
	 *            The host to connect to.
	 ...
	 * 
	 * @param url
	 *            The full url, starting with http://
	 */
	// ===================================================================
	...
	// ===================================================================
	{
		...
		ByteArray ba = ((TextCodec) td.getCopy()).encodeText(rc, 0, rc.length,
				true, null);
		...
		if (bytesToPost != null) {
			...
			bytesToPost.close();
			/*
			 * // For debugging - output eol and a blank line. byte[] ret = new
			 * byte[]{(byte)'\r',(byte)'\n'}; sock.write(ret); sock.write(ret);
			 * sock.flush();
			 */
		}
		//
		...
		//
		CharArray all = ((TextCodec) td.getCopy()).decodeText(ba.data, 0,
				ba.length, true, null);
		if (data == null) {
			data = new SubString();
			lines = new Vector();
		...
		documentProperties.set("response", response);
		{
			...
			if (idx != -1) {
				...
				if (id2 != -1) {
					responseCode = ewe.sys.Convert.toInt(response.substring(
							idx + 1, id2));
				}
			}
		}
		for (int i = 1; i < got; i++) {
			...
			// ewe.sys.Vm.debug(document+": "+name+" = "+value);
	...
	/**
	 * Call this after a successful connection. If the server requested a
	 * redirect (a 3xx code) then this will return an HttpConnection to the new
	 * location which you can connect to again. You must setup any post data or
	 * requestor properties again before re-connecting. Alternatively you could
	 * also call getRedirectTo() and then if that returns a non-null String, you
	 * can call getRedirectedConnection() to get copies. If there is no
	 * redirection required or possible the method will return this same
	 * HttpConnection.
	 */
	// ===================================================================
	...
	/**
	 * Call this after a success connection. If it returns a non-null String
	 * then you need to redirect the connection to the new location. If this
	 * returns non-null you can call getRedirectedConnection() to get a new
	 * HttpConnection that you can use to redirect the connection without having
	 * to setup the connection parameters again.
	 * 
	 * @return null if no redirection is needed, otherwise the location directed
	 *         to.
	 */
	...
	// ===================================================================
	int readInChunkedHeader(Socket connection, ByteArray buff, CharArray chBuff)
			throws IOException
	// ===================================================================
	{
		if (buffer == null)
		...
		chBuff = new AsciiCodec().decodeText(buff.data, 0, buff.length, true,
				chBuff);
	...
	/*
	 * //===================================================================
	 * Handle readInSomeData(final Socket connection,final int numBytes,final
	 * ByteArray dest)
	 * //=================================================================== {
	 * return new ewe.sys.TaskObject(){ protected void doRun(){ try{ ByteArray
	 * ba = dest; if (ba == null) ba = new ByteArray(); ba.clear(); int size =
	 * numBytes; if (buffer == null) buffer = new byte[10240];
	 * handle.setProgress(0.0f); while(size > 0){ int toRead = size >
	 * buffer.length ? buffer.length : size; toRead =
	 ...
	 * handle.setProgress(1.0f); handle.returnValue = ba;
	 * handle.set(Handle.Succeeded); return; }catch(Exception e){
	 * handle.failed(e); }finally{ if (!keepAliveMode || ((handle.check() &
	 * handle.Success) == 0)) connection.close(); } } }.startTask(); }
	 */
	/*
	 * //===================================================================
	 ...
	 * handle.set(Handle.Succeeded); return; }catch(IOException e){
	 * handle.failed(e); } } }.startTask(); }
	 */
	/**
	 * Read in all the data from the Socket.
	 * 
	 * @param connection
	 *            The socket returned by a connect() call.
	 * @return A Handle with which you can monitor the connection. When the
	 *         Handle reports Success, then the returnValue of the Handle will
	 *         be a ewe.util.ByteArray object that holds the data read in.
	 */
	// ===================================================================
	...
	// ===================================================================
	{
		...
		if ("chunked".equals(documentProperties.getValue(encodings, null)))
			return new MemoryStream(true) {
				...
				{
					if (leftInBlock <= 0) {
						leftInBlock = readInChunkedHeader(connectedSocket, ba,
								ca);
						if (leftInBlock <= 0)
							return false;
					}
		...
		// throw new IOException("Cannot get input stream from this!");
			...
			return new PartialInputStream(connectedSocket, length)
					.toInputStream();
	}

	/**
	 * Read in the document body from the Socket. This method blocks until the
	 * complete data is read in. readInData() is a non-blocking version.
	 * 
	 * @param connection
	 *            The socket returned by a connect() call.
	...
	// ===================================================================
	{
		return (ByteArray) waitOnIO(readInData(connection),
				"Error reading data.");
	}

	/**
	 * Read in all the data from the Socket, converting it to text using the
	 * specified codec.
	 * 
	 * @param connection
	 *            The socket returned by a connect() call.
	 * @param documentTextDecoder
	 *            The text codec to use to convert the bytes read in into text.
	 *            If this is null then a simple Ascii codec will be used.
	 * @return A Handle with which you can monitor the connection. When the
	 *         Handle reports Success, then the returnValue of the Handle will
	 *         be a ewe.util.CharArray object that holds the text read in.
	 */
	// ===================================================================
	public Handle readInText(final Socket connection,
			TextCodec documentTextDecoder)
	// ===================================================================
	{
		if (documentTextDecoder == null)
			documentTextDecoder = new AsciiCodec();
		...
		return new ewe.sys.TaskObject() {
			protected void doRun() {
				try {
					...
					handle.returnValue = cc.decodeText(ba.data, 0, ba.length,
							true, null);
	...
	/**
	 * Read in the document body from the Socket. This method blocks until the
	 * complete data is read in. readInText() is a non-blocking version.
	 * 
	 * @param connection
	 *            The socket returned by a connect() call.
	 * @param documentTextDecoder
	 ...
	 */
	// ===================================================================
	public CharArray readText(Socket connection, TextCodec documentTextDecoder)
			throws IOException
	// ===================================================================
	{
		return (CharArray) waitOnIO(
				readInText(connection, documentTextDecoder),
				"Error reading data.");
	}

	/**
	 * Connect asynchronously. This makes the connection, sends the request and
	 * requestor properties reads in the reply and server properties and then
	 * returns the connected Socket ready for for reading in the actual data.
	 * 
	 * @return A Handle used to monitor the connection. When the Handle reports
	 *         a state of Success, then the returnValue of the IOHandle will
	 *         hold the connected socket.
	 */
	// ===================================================================
	 ...
	 *            The text decoder to convert the server and requestor
	 *            properties data into text.
	 * @return A Handle used to monitor the connection. When the Handle reports
	 *         a state of Success, then the returnValue of the Handle will hold
		...
		return new ewe.sys.TaskObject() {
			protected void doRun() {
				while (true) {
					...
					//
					Handle sh = (openSocket != null) ? new Handle(
							Handle.Succeeded, openSocket) : new IOHandle();
					Socket sock = (openSocket != null) ? openSocket
							: new Socket(host, port, (IOHandle) sh);
					try {
						//
	 ...
	 * 
	 * @return A Socket that you can read the data from. The document properties
	 *         will be in the document properties list.
	...
	/**
	 * Connect to the server and save the socket for later use as the
	 * "connectedSocket" field.
	 * 
	 ...
	 * //=================================================================== {
	 * return new ewe.sys.Vm.TaskObject(){ protected void doRun(){ try{ Socket
	 * sock = new Socket(host,port); handle.setFlags(SocketConnected,0);
	 * 
	 * }catch(Exception e){ handle.set(Handle.Failed); return; } }
	 * }.stratTask(); }
	 */
	/*
	 * //===================================================================
	 ...
	 * //=================================================================== {
	 * ewe.sys.Vm.startEwe(args); HttpConnection hp = new
	 * HttpConnection("192.168.0.52",80,"/eweDemo.zip"); Socket sock =
	 * hp.connect(); Handle h = hp.readInData(sock); ProgressBarForm pbf = new
	 * ProgressBarForm(); pbf.showStop = true; pbf.execute(h,"Reading...");
	 * pbf.showMainTask = false; pbf.showSubTask = true; h.waitUntilStopped();
	 * if ((h.check() & h.Success) != 0) ewe.sys.Vm.debug("Success!!");
	 * sock.close(); ewe.sys.Vm.exit(0); }
	 */
	// ##################################################################
}
// ##################################################################
