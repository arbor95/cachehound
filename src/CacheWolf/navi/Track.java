...
public class Track {
	...
	/*
	 * ===== add is currently not used public void add(double lat, double lon) {
	 * TrackPoints[num] = new TrackPoint(lat, lon); num++; } =====
	 */
	/**
	 * throws IndexOutOfBoundsException when track is full
	 */
	public void add(TrackPoint t) {
		if (TrackPoints == null || t == null)
			return;
		TrackPoints[num] = new TrackPoint(t);
		num++;
	}
	/*
	 * ===== loadTrack/saveTrack are currently not used public void
	 * loadTrack(String filename){ // TODO untested! byte [] all =
	 * ewe.sys.Vm.readResource(null,filename); if (all == null) return; // TODO
	 * error handling num = Utils.readInt(all, 0, 4); for (int i=0; i<=num;
	 * i++){ TrackPoints[i].latDec = Double.longBitsToDouble(Utils.readLong(all,
	 * (i*2)*8 + 4)); TrackPoints[i].lonDec =
	 * Double.longBitsToDouble(Utils.readLong(all, (i*2+1)*8 + 4)); } }
	 * 
