...
//				lineLengthString = MyLocale.formatDouble(tmp,"0.000") + Metrics.getUnit(currentUnit);
	...
	public void setBestMap(CWPoint where, boolean loadIfSameScale) {
		...
		if (currentMap == null && newmap == null) {
			// (new MessageBox("Information", "Fďż˝r die aktuelle Position steht keine Karte zur Verfďż˝ng, bitte wďż˝hlen Sie eine manuell", MessageBox.OKB)).execute();
	...
	public Object[] getRectForMapChange(CWPoint ll) {
		...
		if (posCircleX >= 0 && posCircleX <= w && posCircleY >= 0 && posCircleY <= h && ll.isValid()) {
			...
			pY = posCircleY; // TODO eigentlich interessiert, ob nach dem evtl. Kartenwechsel PosCircle on Screen ist. So wie es jetzt ist, kann 2mal der gleiche Aufruf zum laden unterschiedlicher Karten fďż˝hren, wenn vorher PosCircle nicht auf dem SChirm war, nach dem ersten Laden aber schon.
			cll = new CWPoint(ll);
	...
	public void loadMapForAllCaches(){
		...
		if (sur == null) {
			(new MessageBox(MyLocale.getMsg(4207, "Error"), MyLocale.getMsg(4215, "Keine  Caches mit Hďż˝ckchen ausgewďż˝hlt"), FormBase.OKB)).execute();
			return;
		}
...
class MovingMapPanel extends InteractivePanel implements EventListener {
	...
	MenuItem highestResGpsDestMI = new MenuItem(MyLocale.getMsg(4244, "Highest res. containing dest. & cur. position"), new IconAndText(new mImage("res_gps_goto.png"), MyLocale.getMsg(4245, "Highest res. containing dest. & cur. position"), null, CellConstants.RIGHT)); //immer hďż˝chste Auflďż˝sung wďż˝hlen, die akt. Pos. und Ziel enthalten 
	MenuItem highestResolutionMI = new MenuItem(MyLocale.getMsg(4246, "Highest resolution"), new IconAndText(new mImage("res_high.png"), MyLocale.getMsg(4247, "Highest resolution"), null, CellConstants.RIGHT)); //immer hďż˝chste Auflďż˝sung wďż˝hlen 
	MenuItem keepManResolutionMI = new MenuItem(MyLocale.getMsg(4248, "Keep manual resolution"), new IconAndText(new mImage("res_manuell.png"), MyLocale.getMsg(4249, "Keep manual resolution"), null, CellConstants.RIGHT)); // manuell gewďż˝hlte Auflďż˝sung beibehalten  
	// manuell
