...
public class MovingMap extends Form {
	...
	public final static int noGPS = 1; // no GPS-Position marker, manually
										// disconnected
	public final static int ignoreGPS = -1; // ignore even changes in GPS-signal
											// (eg. from lost fix to gotFix)
											// this is wanted when the map is
											// moved manually
	...
	int posCircleX = 0, posCircleY = 0, lastCompareX = Integer.MAX_VALUE,
	...
	boolean forceMapLoad = true; // only needed to force updateposition to try
	...
	public MovingMap(Navigate nav, CacheDB cacheDB) {
		...
		this.backGround = new Color(254, 254, 254); // background must not be
													// black because black is
													// interpreted as
													// transparent and
													// transparent images above
													// (eg trackoverlay) want be
													// drawn in windows-VM, so
													// be care, don|t use white
													// either
		...
		statusImageHaveSignal = new AniImage("position_green" + imagesize
				+ ".png");
		statusImageNoSignal = new AniImage("position_yellow" + imagesize
				+ ".png");
		...
		buttonImageLensActivated = new AniImage("lupe_activated" + imagesize
				+ ".png");
		buttonImageLensActivatedZoomIn = new AniImage("lupe_activated_zin"
				+ imagesize + ".png");
		buttonImageLensActivatedZoomOut = new AniImage("lupe_activated_zout"
				+ imagesize + ".png");
		buttonImageZoom1to1 = new AniImage("zoom1to1" + imagesize + ".png");
		posCircle = new MapSymbol("position_green" + imagesize + ".png",
				"gps-position", new CWPoint());
		...
		bottonImageClose = new AniImage(new Image(closeX.getWidth(), closeX
				.getHeight()));
		...
		DistanceImage.setImage(new Image(MyLocale.getScreenWidth() / 2, fm
				.getHeight()), Color.White); // consider the size of the font
												// used
		...
		ScaleImage.setImage(new Image(MyLocale.getScreenWidth() / 2, fm
				.getHeight()), Color.White); // consider the size of the font
												// used
		...
		// resizeTo(pref.myAppWidth, pref.myAppWidth); // is necessary to
		// initialise mapImage.screenSize
		setGpsStatus(noGPS);
	...
	public void updateFormSize(int w, int h) {
		...
		buttonImageGpsOn.setLocation(w - bottonImageChooseMap.getWidth() - 5,
				bottonImageClose.getHeight() + 20);
		bottonImageChooseMap.setLocation(10, 10);
		directionArrows.setLocation(w / 2 - directionArrows.getWidth() / 2, 10);
		buttonImageZoom1to1.setLocation(
				w - buttonImageZoom1to1.getWidth() - 10, h / 2
						- buttonImageLens.getHeight() / 2
						- buttonImageZoom1to1.getHeight() - 10);
		buttonImageLens.setLocation(w - buttonImageLens.getWidth() - 10, h / 2
				- buttonImageLens.getHeight() / 2);
		buttonImageLensActivated.setLocation(w
				- buttonImageLensActivated.getWidth() - 10, h / 2
				- buttonImageLensActivated.getHeight() / 2);
		buttonImageLensActivatedZoomIn.setLocation(w
				- buttonImageLensActivatedZoomIn.getWidth() - 10, h / 2
				- buttonImageLensActivatedZoomIn.getHeight() / 2);
		buttonImageLensActivatedZoomOut.setLocation(w
				- buttonImageLensActivatedZoomOut.getWidth() - 10, h / 2
				- buttonImageLensActivatedZoomOut.getHeight() / 2);
		DistanceImage.setLocation(0, h - DistanceImage.getHeight());
		ScaleImage.setLocation(w - ScaleImage.getWidth(), h
		...
		if (tracks != null)
			rebuildOverlaySet();
		if (symbols != null) { // TODO: see if the rest of the code works with
								// symbols = null
	 ...
	 * @param lat
	 *            used to create empty maps with correct conversion from lon to
	...
	public void loadMaps(String mapsPath, double lat) {
		...
		resetCenterOfMap();
		InfoBox inf = new InfoBox(MyLocale.getMsg(4201, "Info"), MyLocale
				.getMsg(4203, "Loading list of maps..."));
		inf.exec();
		...
		maps.addEmptyMaps(lat); // the empty maps must be added last, otherwise
								// in method setBestMap, when no mapt is
								// available, a malfunction will happen, see
								// there
	...
	public void updateScale() {
		...
		ScaleImageGraphics.fillRect(0, 0, ScaleImage.location.width,
				ScaleImage.location.height);
		...
		if (currentMap != null) {
			...
			if (metricSystem == Metrics.IMPERIAL) {
				...
				localizedLineLength = Metrics.convertUnit(lineLengthMeters,
						Metrics.METER, smallUnit);
			} else {
			...
			float digits = (float) java.lang.Math.floor(java.lang.Math
					.log(localizedLineLength)
					/ java.lang.Math.log(10.0));
			localizedLineLength = (float) java.lang.Math
					.ceil(localizedLineLength
							/ (float) java.lang.Math.pow(10, digits))
					* (float) java.lang.Math.pow(10, digits);

			if (localizedLineLength >= threshold) {
				currentUnit = bigUnit;
				localizedLineLength = Metrics.convertUnit(lineLengthMeters,
						Metrics.METER, currentUnit);

				digits = (float) java.lang.Math.floor(java.lang.Math
						.log(localizedLineLength)
						/ java.lang.Math.log(10.0));
				localizedLineLength = (float) java.lang.Math
						.ceil(localizedLineLength
			...
			String lineLengthString = Convert
					.toString((int) localizedLineLength)
			...
			if (digits < 0) {
				...
				lineLengthString = tmp.toString(decimals + 2, decimals, 0)
						+ Metrics.getUnit(currentUnit);
				// lineLengthString = MyLocale.formatDouble(tmp,"0.000") +
				// Metrics.getUnit(currentUnit);
			}

			lineLengthMeters = Metrics.convertUnit(localizedLineLength,
					currentUnit, Metrics.METER);

			int lineLengthPixels = (int) java.lang.Math.round(lineLengthMeters
					/ currentMap.scale);
			int backgroundStartX = ScaleImage.location.width
					- (lineLengthPixels + fm.getTextWidth(lineLengthString) + 7);

			ScaleImageGraphics.setColor(new Color(250, 250, 250));
			ScaleImageGraphics.fillRect(backgroundStartX, 0,
					ScaleImage.location.width - backgroundStartX,
					ScaleImage.location.height);

			ScaleImageGraphics.setPen(new Pen(Color.DarkBlue, Pen.SOLID, 3));
			ScaleImageGraphics.drawLine(backgroundStartX + 2,
					ScaleImage.location.height / 2, backgroundStartX + 2
							+ lineLengthPixels, ScaleImage.location.height / 2);
			ScaleImageGraphics.setColor(Color.DarkBlue);
			ScaleImageGraphics.drawText(lineLengthString, backgroundStartX
					+ lineLengthPixels + 5, 0);
		}
	...
	public void updateDistance(boolean repaint) {
		...
		DistanceImageGraphics.fillRect(0, 0, DistanceImage.location.width,
				DistanceImage.location.height);
		if (gotoPos != null && posCircle.where.isValid()) {
			...
			if (currentDistance != lastDistance) {
				...
				if (metricSystem == Metrics.IMPERIAL) {
					...
					smallUnit = Metrics.FEET;
					threshold = 0.1;
					localizedDistance = Metrics.convertUnit(currentDistance,
				...
				if (dd.value >= threshold) {
					d = MyLocale.formatDouble(dd, "0.000")
							+ Metrics.getUnit(bigUnit);
				} else {
				...
				DistanceImageGraphics.fillRect(0, 0, backgroundWidth,
						DistanceImage.location.height);

				DistanceImageGraphics.setColor(Color.DarkBlue);
				DistanceImageGraphics.drawText(d, 2, 0);

				DistanceImageGraphics.drawImage(DistanceImage.image, null,
						Color.LightBlue, 0, 0, DistanceImage.location.width,
						DistanceImage.location.height); // changing the mask
														// forces graphics to
														// copy from
														// image._awtImage to
														// image.bufferedImage,
														// which is displayed
				DistanceImageGraphics.drawImage(DistanceImage.image, null,
						Color.White, 0, 0, DistanceImage.location.width,
						DistanceImage.location.height); // these 2 commands are
			...
			DistanceImageGraphics.drawImage(DistanceImage.image, null,
					Color.LightBlue, 0, 0, DistanceImage.location.width,
					DistanceImage.location.height); // changing the mask forces
													// graphics to copy from
													// image._awtImage to
													// image.bufferedImage,
													// which is displayed
			DistanceImageGraphics.drawImage(DistanceImage.image, null,
	...
	public void forceMapLoad() {
		forceMapLoad = true;
		updatePosition(posCircle.where); // this sets forceMapLoad to false
											// after loading a map
	}
	...
	public final FormFrame myExec() {
		...
		if (tracks != null && tracks.size() > 0
			...
			rebuildOverlaySet(); // show points which where added when MavingMap
									// was not running
	...
	public void addOverlaySet() {
		...
		try {
			...
			// creation of Overlayset will be done in updateOverlayPos if tracks
			// != null
		}
	}
	...
	public void addMissingOverlays() {
		if (currentMap == null || (!posCircle.where.isValid()) || width == 0
				|| height == 0)
			return; // height == 0 happens if this is called before the form ist
		...
		for (int yi = 0; yi < 3; yi++) {
			for (int xi = 0; xi < 3; xi++) {
				...
				if (TrackOverlays[i] == null) {
					TrackOverlays[i] = new TrackOverlay(ScreenXY2LatLon(
	...
	public void rearangeOverlays() {
		...
		if (TrackOverlays[1].isOnScreen()) { // oben raus
			TrackOverlaySetCenterTopLeft.set(ScreenXY2LatLon(oldp.x, oldp.y - 2
					* height));
			destroyOverlay(6);
			destroyOverlay(7);
			destroyOverlay(8);
			...
			if (TrackOverlays[3].isOnScreen()) { // links raus
				TrackOverlaySetCenterTopLeft.set(ScreenXY2LatLon(oldp.x - 2
						* width, oldp.y));
				destroyOverlay(2);
				destroyOverlay(5);
				destroyOverlay(8);
				...
				if (TrackOverlays[5].isOnScreen()) { // rechts raus
					...
					if (TrackOverlays[7].isOnScreen()) { // unten raus
						...
						destroyOverlay(4);
						destroyOverlay(5);
					} else { // it is important to test for diagonal only if the
								// other didn't match
						if (TrackOverlays[0].isOnScreen()) { // links oben raus
							...
							if (TrackOverlays[2].isOnScreen()) { // rechts oben
																	// raus
								TrackOverlaySetCenterTopLeft
										.set(ScreenXY2LatLon(
												oldp.x + 2 * width, oldp.y - 2
														* height));
								destroyOverlay(6);
								mmp.removeImage(TrackOverlays[2]);
								...
								if (TrackOverlays[6].isOnScreen()) { // links
									...
									if (TrackOverlays[8].isOnScreen()) { // rechts
										...
										for (int i = 0; i < TrackOverlays.length; i++) {
											destroyOverlay(i);
											TrackOverlaySetCenterTopLeft = ScreenXY2LatLon(
													100, 100);
										} // this happens if a position jump
											// occured
								}
							}
						}
	...
	public void updateOverlayPos() {
		...
		if (TrackOverlays == null || TrackOverlays[4] == null)
			...
			updateOverlayOnlyPos();
			if (TrackOverlays[0].locAlways.x > 0
					|| TrackOverlays[2].locAlways.x < 0
					|| TrackOverlays[0].locAlways.y > 0
					|| TrackOverlays[8].locAlways.y < 0) { // testForNeedToRearange
				rearangeOverlays();
				addMissingOverlays();
	...
	public void resetCenterOfMap() {
		if (width != 0) {
			...
			posCircleX = pref.myAppWidth / 2; // maybe this could /should be
												// repleced to windows size
			posCircleY = pref.myAppHeight / 2;
		}
		posCircle.hidden = false;
		posCircle.move(posCircleX - posCircle.getWidth() / 2, posCircleY
				- posCircle.getHeight() / 2); // posCircle.setLocation caused a
												// problem -> hiding the
	...
	public void moveScreenXYtoLatLon(Point s, CWPoint c, boolean repaint) {
		...
		if (mmp != null && mmp.mapImage != null)
			mmp.mapImage.move(mappos.x - onscreenpos.x + s.x, mappos.y
					- onscreenpos.y + s.y);
	...
	/**
	 * call this if the map moved on the screen (by dragging) this routine will
	 * adjust (move accordingly) all other symbols on the screen
	 * 
	 * @param diffX
	 * @param diffY
	 */
	...
	/**
	 * get upper left corner of map on window this is called when the map needs
	 * to be moved / the position of the map is wanted the map-position is
	 * calculated relativ to posCircle (x,y and lat/lon) returns the same as
	 * mmp.mapImage.getLocation(mapPos); but also works if mmp == null and is
	 * used to move the map to the correct point
	 * 
	 * @return
	 */
	...
	public Point getXYonScreen(TrackPoint ll) {
		...
		Point mapPos = getMapPositionOnScreen();
		// Vm.debug("getXYinMap, posCiLat: "+posCircleLat+"poscLOn: "+
		// posCircleLon+"gotoLat: "+ lat + "gotoLon: "+
		// lon+" mapPosX: "+mapPos.x+"mapposY"+mapPos.y);
	...
	public MapSymbol addSymbol(String pName, String filename, CWPoint where) {
		...
		ms.setLocation(pOnScreen.x - ms.getWidth() / 2, pOnScreen.y
				- ms.getHeight() / 2);
		symbols.add(ms);
		mmp.addImage(ms);
		return ms;
	...
	public MapSymbol addSymbol(String pName, Object mapObject, String filename,
			CWPoint where) {
		if (symbols == null)
			symbols = new Vector();
	...
	public void addSymbolIfNecessary(String pName, Object mapObject,
			Image imSymb, CWPoint where) {
	...
	public void addSymbol(String pName, Object mapObject, Image imSymb,
			CWPoint ll) {
		if (symbols == null)
			symbols = new Vector();
	...
	public void destChanged(CWPoint d) {
		if (!running || (d == null && gotoPos == null)
				|| (d != null && gotoPos != null && gotoPos.where.equals(d)))
			return;
		removeGotoPosition();
		if (d == null || !d.isValid())
			return;
		...
		forceMapLoad = true;
		if (this.width != 0)
			updatePosition(posCircle.where); // dirty hack: if this.width == 0,
	...
	public void updateOnlyPosition(CWPoint where, boolean updateOverlay) {
		...
		// Vm.debug("mapy = " + mapy);
		if (forceMapLoad
				|| (java.lang.Math.abs(lastRepaintMapPos.x - mapPos.x) > 1 || java.lang.Math
						.abs(lastRepaintMapPos.y - mapPos.y) > 1)) {
		...
		if (forceMapLoad || wantMapTest || screenNotCompletlyCovered) { // if
			...
			if (forceMapLoad
					|| (java.lang.Math.abs(lastCompareX - mapPos.x) > this.width / 10 || java.lang.Math
							.abs(lastCompareY - mapPos.y) > this.height / 10)) {
	...
	public void updateGps(int fix) {
		...
		if ((fix > 0) && (myNavigation.gpsPos.getSats() >= 0)) { // TODO is
			...
			directionArrows.setDirections((float) myNavigation.gpsPos
					...
					(float) myNavigation.gpsPos.getBear());
	...
	boolean wantMapTest = true; // if true updateposition calls setBestMap
								// regulary even if the currentmap covers the
								// whole screen
	public final static int NORMAL_KEEP_RESOLUTION = 1; // keeps the choosen
														...
														// the PosCircle - it
														// changes the
														// resolution if no such
														// map is available. It
														// wil cahnge back to
														// the wanted scale as
														// soon as a map becomes
														// available (through
														// movement of the
														// GPS-receiver)
	...
	/**
	 * loads the best map for lat/lon according to mapChangeModus lat/lon will
	 * be at the screen-pos of posCircle when posCircle is not on the screen
	 * (shifted outside my the user) then this routine uses the centre of the
	 * screen to find the best map but anyway the map will be adjusted (moved)
	 * relativ to posCircle when a better map was found the called method
	 * updateposition will set posCirleLat/-Lon to lat/lon.
	 * 
	 * @param lat
	 * @param lon
	 * @param loadIfSameScale
	 *            false: will not change the map if the better map has the same
	 *            scale as the current - this is used not to change the map if
	 *            it covers already the screen completely true: willchange the
	 *            map, regardless of change in scale
	 */
	public void setBestMap(CWPoint where, boolean loadIfSameScale) {
		...
		// if (mapChangeModus == 0) mapChangeModus =
		// HIGHEST_RESOLUTION_GPS_DEST;
		wantMapTest = true;
		...
		case HIGHEST_RESOLUTION_GPS_DEST:
			if (gotoPos != null && GpsStatus != noGPS
					&& posCircle.where.isValid()) {
				if ((!posCircleOnScreen)
						&& (lastHighestResolutionGPSDestScale > 0)) {
					newmap = maps.getBestMap(cll, screen,
							lastHighestResolutionGPSDestScale, false);
				} else {
					...
					if (newmap == null)
						newmap = maps.getBestMap(cll, screen,
								10000000000000000000000000000000000f, false); // use
																				// map
																				// with
																				// most
																				// available
		...
		default:
			(new MessageBox(MyLocale.getMsg(4207, "Error"), MyLocale.getMsg(
					...
					+ mapChangeModus, FormBase.OKB)).execute();
			break;
		}
		if (newmap != null
				&& (currentMap == null || !currentMap.mapName
						.equals(newmap.mapName))) {
			if (loadIfSameScale
					|| !MapsList.scaleEquals(currentMap.scale
							/ currentMap.zoomFactor, newmap)) {
		...
		if (currentMap == null && newmap == null) {
			// (new MessageBox("Information",
			// "Fďż˝r die aktuelle Position steht keine Karte zur Verfďż˝ng, bitte wďż˝hlen Sie eine manuell",
			// MessageBox.OKB)).execute();
			posCircle.where.set(cll); // choosemap calls setmap with
										// posCircle-coos
			try {
				setMap(((MapListEntry) maps.elementAt(maps.getCount() - 4))
				...
				(new MessageBox(
						...
						MyLocale
								.getMsg(
										4209,
										"setBestMap: problem in: setMap( ((MapListEntry)maps.elementAt(maps.getCount() - 4)).getMap(), lat, lon) lat/lon:")
								+ where.toString(), FormBase.OKB)).exec();
			}
			while (currentMap == null) { // this actually cannot happen, but
											// maybe in case of an inconstistent
											// code change (esp. regarding empty
											// maps)
				...
				if (currentMap == null)
					(new MessageBox(
							...
							MyLocale
									.getMsg(
											4210,
											"Moving map cannot run without a map - please select one. \n You can select an empty map"),
							FormBase.OKB)).execute();
			}
		}
		inBestMap = false;
	...
	/**
	 * method to get a point on the screen which must be included in the map the
	 * map methods are looking for. If the poscircle is on the screen this will
	 * be that point. If it is outside then the centre of the screen will be
	 * used.
	 * 
	...
	public Object[] getRectForMapChange(CWPoint ll) {
		int w = (width != 0 ? width : pref.myAppWidth); // width == 0 happens if
														// this routine is run
														// before the windows is
														// on the screen
		int h = (height != 0 ? height : pref.myAppHeight);
		int pX, pY;
		CWPoint cll;
		Boolean posCircleOnScreen = java.lang.Boolean.FALSE;
		if (posCircleX >= 0 && posCircleX <= w && posCircleY >= 0
				&& posCircleY <= h && ll.isValid()) {
			posCircleOnScreen = java.lang.Boolean.TRUE;
			pX = posCircleX; // posCircle is inside the screen
			pY = posCircleY; // TODO eigentlich interessiert, ob nach dem evtl.
								...
								// wenn vorher PosCircle nicht auf dem SChirm
								// war, nach dem ersten Laden aber schon.
			cll = new CWPoint(ll);
		} else { // when posCircle out of screen - use centre of screen as point
	...
	public void loadMoreDetailedMap(boolean betterOverview) {
		...
		MapInfoObject m = maps.getMapChangeResolution(cll, screen,
				currentMap.scale * currentMap.zoomFactor, !betterOverview);
		if (m != null) {
			...
			(new MessageBox(MyLocale.getMsg(4207, "Error"), MyLocale.getMsg(
					4211, "No ")
					+ (betterOverview ? MyLocale.getMsg(4212, "less")
							: MyLocale.getMsg(4213, "more"))
	...
	public void loadMapForAllCaches() {
		...
		if (sur == null) {
			(new MessageBox(MyLocale.getMsg(4207, "Error"), MyLocale.getMsg(
					4215, "Keine  Caches mit Hďż˝ckchen ausgewďż˝hlt"),
					FormBase.OKB)).execute();
			return;
		}
		...
		if (newmap == null) { // no map is covering any area of the caches ->
								// zoom an empty map to cover all caches on
								// screen
			try {
				Object[] s = getRectForMapChange(posCircle.where);
				// CWPoint cll = (CWPoint) s[0];
				Rect screen = (Rect) s[1];
				float neededscalex = (float) (sur.topleft.getDistance(
						sur.topleft.latDec, sur.buttomright.lonDec) * 1000 / (screen.width - 15)); // 15
																									// for
																									// the
																									// size
																									// of
																									...
																									// cache
																									// image
				float neededscaley = (float) (sur.topleft.getDistance(
						sur.buttomright.latDec, sur.topleft.lonDec) * 1000 / (screen.height - 15)); // 15
																									// for
																									// the
																									// size
																									// of
																									...
																									// cache
																									// image
				newmap = ((MapListEntry) maps.elementAt(maps.getCount() - 4))
						.getMap(); // beware: "-4" only works if the empty maps
									// were added last see MapsList.addEmptyMaps
				newmap.zoom(newmap.scale
						* newmap.zoomFactor
				...
				(new MessageBox(
						...
						MyLocale
								.getMsg(
										4279,
										"loadMapForAllCaches: IO-Exception in: newmap = ((MapListEntry)maps.elementAt(maps.getCount() - 4)).getMap();"),
						FormBase.OKB)).exec();
			}
		}
		boolean saveGpsIgnStatus = dontUpdatePos;
	...
	public void setGpsStatus(int status) {
		...
		mapMoved(0, 0); // positions the posCircle correctly accourding to its
						// size (which can change when the image changes, e.g.
	...
	public void SnapToGps() {
		...
		dontUpdatePos = false;
		ignoreGps = false;
		lastCompareX = Integer.MAX_VALUE; // neccessary to make updateposition
											// to test if the current map is the
											// best one for the GPS-Position
		lastCompareY = Integer.MAX_VALUE;
		autoSelectMap = true;
		forceMapLoad = true;
		showMap();
	 ...
	 * @param newmap
	 * @param lat
	 *            move map so that lat/lon is in the centre / -361: don't adust
	 *            to lat/lon
	 * @param lon
	 *            -361: don't adust to lat/lon
	 */
	public void setMap(MapInfoObject newmap, CWPoint where) {
		if (currentMap != null && newmap.mapName.equals(currentMap.mapName)
				&& !forceMapLoad) { // note: newmap.mapName ==
									// currentMap.mapName won't work because
									// they are different String containing the
									// same text
			updateOnlyPosition(where, true);
			return;
		}
		...
		dontUpdatePos = true; // make updatePosition ignore calls during loading
								// new map
		InfoBox inf;
		inf = new InfoBox(MyLocale.getMsg(4201, "Information"), MyLocale
				.getMsg(4216, "Loading map..."));
		inf.show();
		inf.waitUntilPainted(100);
		try {
			...
			lastCompareY = Integer.MAX_VALUE;
			if (mmp.mapImage != null) {
				// Vm.debug("free: "+Vm.getUsedMemory(false)+"classMemory: "+Vm.getClassMemory()+
				// "after garbage collection: "+Vm.getUsedMemory(false));
			...
			updateScale();
			inf.close(0); // this doesn't work in a ticked-thread in the ewe-vm.
			...
			rebuildOverlaySet();
			updateOnlyPosition(where, false); // TODO this doesn't work
												// correctly if the resolution
												// changed, I guess because the
												// pixels of PosCircle will be
												// interpreted from the new
												// resolution, but should be
												// interpreted using the old
												// resolution to test: select a
												// map with a much greater value
												// of m per pixel manually
			inf.close(0);
			Vm.showWait(false);
			(new MessageBox(
					MyLocale.getMsg(4207, "Error"),
	...
	public void zoomScreenRect(Point firstclickpoint, int w, int h) {
		...
		if (w > 0) { // zoom in
			zoomFactor = java.lang.Math.min((float) this.width / (float) w,
					(float) this.height / (float) h);
		} else { // zoom out
			w = java.lang.Math.abs(w);
			firstclickpoint.x = firstclickpoint.x - w; // make firstclickedpoint
														// the upper left corner
			zoomFactor = java.lang.Math.max((float) w / (float) this.width,
					(float) h / (float) this.height);
		}
		// calculate rect in unzoomed image in a way that the centre of the new
		// image is the centre of selected area but give priority to the
		...
		Point mappos = getMapPositionOnScreen();
		int xinunscaledimage = (int) ((firstclickpoint.x - mappos.x + w / 2)
				/ currentMap.zoomFactor + currentMap.shift.x - newImageWidth / 2);
		int yinunscaledimage = (int) ((firstclickpoint.y - mappos.y + h / 2)
				/ currentMap.zoomFactor + currentMap.shift.y - newImageHeight / 2);
		Rect newImageRect = new Rect(xinunscaledimage, yinunscaledimage,
				newImageWidth, newImageHeight);
		if (mapImage1to1 != null && mmp.mapImage != null
				&& mapImage1to1.image != null) {
			...
			if (newImageRect.x + newImageRect.width >= mapImage1to1.getWidth())
				newImageRect.x = mapImage1to1.getWidth() - newImageWidth; // align
																			...
																			// right
																			// overlaping
			if (newImageRect.y + newImageRect.height >= mapImage1to1
					.getHeight())
		...
		zoomFromUnscaled(zoomFactor * currentMap.zoomFactor, newImageRect,
				center);
	}
	...
	public void zoom1to1() {
		...
		if (mapImage1to1 != null)
			zoomFromUnscaled(1, new Rect(0, 0, mapImage1to1.getWidth(),
					mapImage1to1.getHeight()), center);
		else
	 ...
	 * @param center
	 */
	public void zoomFromUnscaled(float zoomFactor, Rect newImageRect,
			CWPoint center) {
		Vm.showWait(this, true);
		...
		if (mapImage1to1 != null) {
			...
			try {
				if (zoomFactor == 1)
					...
					tmp = new MapImage(mapImage1to1.scale(
							(int) (newImageRect.width * zoomFactor),
							(int) (newImageRect.height * zoomFactor),
							newImageRect, 0));
	...
	/*
	 * public void gotFocus(int how) { super.gotFocus(how); Dimension ws =
	 * getSize(null); onWindowResize(ws.width, ws.height); Vm.debug(ws.width +
	 * " h: "+ws.height); this.setPreferredSize(width, height) }
	 */
	public void onEvent(Event ev) {
		...
		if (ev instanceof KeyEvent
				...
				&& ((((KeyEvent) ev).key == IKeys.ESCAPE)
						|| (((KeyEvent) ev).key == IKeys.ENTER) || (((KeyEvent) ev).key == IKeys.ACTION))) {
			this.close(0);
			ev.consumed = true;
		}
		super.onEvent(ev);
...
class MovingMapPanel extends InteractivePanel implements EventListener {
	...
	Menu mapsMenu;
	MenuItem selectMapMI = new MenuItem(MyLocale.getMsg(4234,
			"Select a map manually$s"), new IconAndText(new mImage(
			"map_open.png"), MyLocale.getMsg(4235, "Select a map manually"),
			null, CellConstants.RIGHT));
	MenuItem changeMapDirMI = new MenuItem(MyLocale.getMsg(4236,
			"Change map directory$c"), new IconAndText(
			new mImage("map_cd.png"), MyLocale.getMsg(4237,
					"Change map directory"), null, CellConstants.RIGHT));
	MenuItem showMapMI = new MenuItem(MyLocale.getMsg(4238, "Show map"),
			new IconAndText(new mImage("map_on.png"), MyLocale.getMsg(4239,
					"Show map"), null, CellConstants.RIGHT));
	MenuItem hideMapMI = new MenuItem(MyLocale.getMsg(4240, "Hide map"),
			new IconAndText(new mImage("map_off.png"), MyLocale.getMsg(4241,
	...
	MenuItem highestResGpsDestMI = new MenuItem(MyLocale.getMsg(4244,
			...
			new mImage("res_gps_goto.png"), MyLocale.getMsg(4245,
					"Highest res. containing dest. & cur. position"), null,
			CellConstants.RIGHT)); // immer hďż˝chste Auflďż˝sung wďż˝hlen, die
									// akt. Pos. und Ziel enthalten
	MenuItem highestResolutionMI = new MenuItem(MyLocale.getMsg(4246,
			"Highest resolution"), new IconAndText(new mImage("res_high.png"),
			MyLocale.getMsg(4247, "Highest resolution"), null,
			CellConstants.RIGHT)); // immer hďż˝chste Auflďż˝sung wďż˝hlen
	MenuItem keepManResolutionMI = new MenuItem(MyLocale.getMsg(4248,
			"Keep manual resolution"), new IconAndText(new mImage(
			"res_manuell.png"),
			MyLocale.getMsg(4249, "Keep manual resolution"), null,
			CellConstants.RIGHT)); // manuell gewďż˝hlte Auflďż˝sung beibehalten
	// manuell
	...
	MenuItem moveToDestMI = new MenuItem(MyLocale.getMsg(4260,
			...
			CellConstants.RIGHT)); // * Karte zum Ziel verschieben (und ggf.
									// entsprechende Karte laden)
	MenuItem moveToGpsMI = new MenuItem(MyLocale.getMsg(4262,
			"Move to GPS position"), new IconAndText(
			new mImage("move2gps.png"), MyLocale.getMsg(4263,
					"Move to GPS position"), null, CellConstants.RIGHT));
	MenuItem moveToCenterMI = new MenuItem(MyLocale.getMsg(4264,
			"Move to centre"), new IconAndText(new mImage("move2center.png"),
			MyLocale.getMsg(4265, "Move to centre"), null, CellConstants.RIGHT));
	...
	public void onPenEvent(PenEvent ev) {
		...
		if (!mm.zoomingMode && ev.type == PenEvent.PEN_DOWN
				&& ev.modifiers == PenEvent.RIGHT_BUTTON) {
		...
		if (mm.zoomingMode && ev.type == PenEvent.PEN_UP) {
			...
			if (java.lang.Math.abs(lastZoomWidth) < 15
					|| java.lang.Math.abs(lastZoomHeight) < 15) {
				repaintNow();
				return; // dont make to big zoom jumps - it is most probable not
						// an intentional zoom
			}
		...
		if (mm.zoomingMode
				&& paintingZoomArea
				&& (ev.type == PenEvent.PEN_MOVED_ON
						|| ev.type == PenEvent.PEN_MOVE || ev.type == PenEvent.PEN_DRAG)) {
			int left, top;
			...
			if ((lastZoomWidth <= 0) && (ev.x - saveMapLoc.x > 0)) { // changed
				...
				this.repaintNow(dr, new Rect(
						mm.buttonImageLensActivatedZoomIn.location.x,
						mm.buttonImageLensActivatedZoomIn.location.y,
			...
			if ((lastZoomWidth >= 0) && (ev.x - saveMapLoc.x < 0)) { // changed
				...
				this.repaintNow(dr, new Rect(
						mm.buttonImageLensActivatedZoomOut.location.x,
						mm.buttonImageLensActivatedZoomOut.location.y,
						mm.buttonImageLensActivatedZoomOut.getWidth(),
						mm.buttonImageLensActivatedZoomOut.getHeight()));
			}
			this.repaintNow(dr, new Rect(left, top, java.lang.Math
					.abs(lastZoomWidth) + 4,
					java.lang.Math.abs(lastZoomHeight) + 4));
			...
			dr.drawRect(left, top, java.lang.Math.abs(lastZoomWidth),
					java.lang.Math.abs(lastZoomHeight), 0); // bug in ewe:
															// thickness
															// parameter is
															// ignored
		}
	...
	public void moveMap(int diffX, int diffY) {
		...
		if (mapImage != null) {
			...
			// if (mm.mapHidden) mapImage.properties |= AniImage.IsInvisible; //
			// this is neccesarry because move will unhide the map if the coos
			// show that the map is on the screen
		}
	...
	public void chooseMap() {
		...
		if (mm.myNavigation.gpsPos.Fix > 0)
			gpspos = new CWPoint(mm.myNavigation.gpsPos.latDec,
					mm.myNavigation.gpsPos.lonDec);
		else
			gpspos = null;
		...
		if (l.execute() == FormBase.IDOK) {
			...
			if (l.selectedMap.isInBound(mm.posCircle.where)
				...
				if (mm.currentMap.fileNameWFL.length() > 0)
					mm.setCenterOfScreen(l.selectedMap.center, true); // if map
																		// has
																		// an
																		// image
				mm.setResModus(MovingMap.NORMAL_KEEP_RESOLUTION);
				...
				// mm.currentMap.affine[2]*posCXY.y + mm.currentMap.affine[4];
				// mm.posCircleX = 0; // place map to the upper left corner of
				// windows
				// mm.posCircleY = 0;
	...
	public void imageClicked(AniImage which, Point pos) {
		if (which == mm.bottonImageChooseMap) {
			...
			if (!mm.noMapsAvailable) {
				if (mm.mapHidden)
					mapsMenu.addItem(showMapMI);
				else
					mapsMenu.addItem(hideMapMI);
			}
			...
			case MovingMap.NORMAL_KEEP_RESOLUTION:
				keepManResolutionMI.modifiers |= MenuItem.Checked;
				break;
			case MovingMap.HIGHEST_RESOLUTION:
				highestResolutionMI.modifiers |= MenuItem.Checked;
				break;
			case MovingMap.HIGHEST_RESOLUTION_GPS_DEST:
				highestResGpsDestMI.modifiers |= MenuItem.Checked;
				break;
			}
			...
			if (mm.gotoPos != null)
				moveToDestMI.modifiers &= ~MenuItem.Disabled;
			else
				moveToDestMI.modifiers |= MenuItem.Disabled;
			mapsMenu.addItem(moveToDestMI);
			if (Global.getPref().curCentrePt.isValid())
				moveToCenterMI.modifiers &= ~MenuItem.Disabled;
			else
				moveToCenterMI.modifiers |= MenuItem.Disabled;
			...
			// m.set(Menu., status)
			mapsMenu.exec(this, new Point(which.location.x, which.location.y),
					this);
		}
	...
	public void snapToGps() {
		mm.myNavigation.startGps(mm.pref.logGPS, Convert
				.toInt(mm.pref.logGPSTimer));
		mm.SnapToGps();
	}
	...
	public void penHeld(Point p) {
		...
		if (!mm.zoomingMode) {
			// ( (ev.type == PenEvent.PEN_DOWN) && ((PenEvent)ev).modifiers ==
			// PenEvent.RIGHT_BUTTON)
			// || ((ev.type == PenEvent.RIGHT_BUTTON) ) )) ---> these events are
			// not posted --> this overridering is the only solution
			...
			if (!(mm.directionArrows.onHotArea(p.x, p.y))) {
				...
				if (clickedOnImage != null
						&& clickedOnImage instanceof MapSymbol) {
					clickedCache = ((CacheHolder) ((MapSymbol) clickedOnImage).mapObject);
					if (clickedCache != null) {
						openCacheDescMenuItem = new MenuItem(
								MyLocale.getMsg(4270, "Open")
										+ " '"
										+ (clickedCache.getCacheName().length() > 0 ? clickedCache
												.getCacheName()
												: clickedCache.getWayPoint())
										+ "'$o"); // clickedCache == null can
													// happen if clicked on the
													// goto-symbol
						kontextMenu.addItem(openCacheDescMenuItem);
						gotoCacheMenuItem = new MenuItem(
								MyLocale.getMsg(4279, "Goto")
										+ " '"
										+ (clickedCache.getCacheName().length() > 0 ? clickedCache
												.getCacheName()
												: clickedCache.getWayPoint())
										+ "'$g"); // clickedCache == null can
													// happen if clicked on the
													// goto-symbol
						kontextMenu.addItem(gotoCacheMenuItem);
						if (Global.mainForm.cacheListVisible) {
							addCachetoListMenuItem = new MenuItem(MyLocale
									.getMsg(199, "Add to cachetour"));
	...
	public void onEvent(Event ev) {
		if (mapsMenu != null && ev instanceof PenEvent
				&& ev.type == PenEvent.PEN_DOWN && ev.target == this) {
			mapsMenu.close();
			mapsMenu = null;
		}
		if (kontextMenu != null && ev instanceof PenEvent
				&& ev.type == PenEvent.PEN_DOWN && ev.target == this) {
			kontextMenu.close();
			kontextMenu = null;
		}
		...
		if (ev instanceof MenuEvent) {
			if (ev.target == mapsMenu) {
				if (ev.type == MenuEvent.ABORTED
						|| ev.type == ControlEvent.CANCELLED
						|| ev.type == ControlEvent.FOCUS_OUT)
			...
			if (ev.target == kontextMenu) {
				if ((((MenuEvent) ev).type == MenuEvent.SELECTED)) {
					...
					if (action == gotoMenuItem) {
						kontextMenu.close();
						mm.myNavigation.setDestination(mm.ScreenXY2LatLon(
								saveMapLoc.x, saveMapLoc.y));
					}
					...
					if (action == addCachetoListMenuItem) {
						kontextMenu.close();
						Global.mainForm.cacheList.addCache(clickedCache
								.getWayPoint());
					}
...
class ListBox extends Form {
	...
	public ListBox(Vector maps, CWPoint Gps, CWPoint gotopos,
			MapInfoObject curMap) {
		this.title = MyLocale.getMsg(4271, "Maps");
		// if (Gui.screenIs(Gui.PDA_SCREEN)) this.setPreferredSize(200,100);
		// else
		// set width to screenwidth *3/4 but to at least 240 if the screen is
		// big engough for 240px width
		this.setPreferredSize(java.lang.Math.max(
				MyLocale.getScreenWidth() * 3 / 4, java.lang.Math.min(240,
						MyLocale.getScreenWidth())),
				MyLocale.getScreenHeight() * 3 / 4);
		this.maps = maps;
		MapInfoObject map;
		MapListEntry ml;
		String cmp;
		...
		if (gotopos != null && Gps != null) {
			list.addItem(MyLocale.getMsg(4272,
					"--- Maps containing GPS and goto pos. ---"));
			row++;
			cmp = "FF1"
					+ (new Area(new CWPoint(Gps.latDec, Gps.lonDec), gotopos))
							.getEasyFindString();
			for (int i = 0; i < maps.size(); i++) {
				...
				if (map.isInBound(Gps.latDec, Gps.lonDec)
						&& map.isInBound(gotopos)) {
					list.addItem(i + ": " + map.mapName);
					row++;
					inList[i] = true;
					if (!curMapFound && curMap != null
							&& map.mapName.equals(curMap.mapName)) {
						oldmap = row;
						curMapFound = true;
					}
				} else
		...
		if (Gps != null) {
			list.addItem(MyLocale.getMsg(4273,
					"--- Maps containing curr. position ---"));
			row++;
			cmp = "FF1"
					+ Area.getEasyFindString(
							new CWPoint(Gps.latDec, Gps.lonDec), 30);
			for (int i = 0; i < maps.size(); i++) {
				ml = (MapListEntry) maps.get(i);
				try {
					if (!Area.containsRoughly(ml.sortEntryBBox, cmp))
		...
		for (int i = 0; i < maps.size(); i++) {
			...
			if (!inList[i]) {
				...
				row++;
				if (!curMapFound && curMap != null
						&& ml.filename.equals(curMap.mapName)) {
					oldmap = row;
					curMapFound = true;
				}
			}
...
class ArrowsOnMap extends AniImage {
	...
	public void setDirections(float gd, float sd, float md) {
		if (java.lang.Math.abs(gotoDir - gd) > 1 // to save cpu-usage only
													// update if the is a change
													// of directions of more
													// than 1 degree
	...
	public void doDraw(Graphics g, int options) {
		...
		/*
		 * if (!dirsChanged) {
		 * g.drawImage(image,mask,transparentColor,0,-minY,location
		 * .width,location.height); // the transparency with a transparent color
		 * doesn't work in ewe-vm for pocketpc, it works in java-vm, ewe-vm on
		 * pocketpc2003 return; } dirsChanged = false; //super.doDraw(g,
		 * options); draw.setColor(Color.White); draw.fillRect(0, 0,
		 * location.width, location.height); minY = Integer.MAX_VALUE;
		 * drawArrows(draw);
		 * draw.drawImage(image,mask,Color.DarkBlue,0,0,location
		 * .width,location.height); // this trick (note: wrong transparentColor)
		 * forces a redraw
		 * g.drawImage(image,mask,transparentColor,0,-minY,location
	...
	private void makeArrows() {
		...
		if (sunDir < 360 && sunDir > -360) {
			...
			sunDirArrow = null;
		if (java.lang.Math.abs(map.rotationRad) > 1.5 / 180 * java.lang.Math.PI) { // show
																					// northth
																					// arrow
																					// only
																					// if
	...
	private void makeArrow(Point[] arrow, float angle, float scale) {
		...
		arrow[1].x = centerX
				+ new Float(centerX * java.lang.Math.sin(angleRad) * scale)
						.intValue();
		arrow[1].y = centerY
				- new Float(centerY * java.lang.Math.cos(angleRad) * scale)
						.intValue();
