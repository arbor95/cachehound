...
/**
 * class to read, save and do the calculations for calibrated and calibrating
 * maps start offset for language file: 4300
 * 
 * @author pfeffer
 * 
 */
public class MapInfoObject extends Area {
	...
	/**
	 * constructes an MapInfoObject without an associated map but with 1 Pixel =
	 * scale meters
	 */
	public MapInfoObject(double scalei, double lat) {
		super(new CWPoint(1, 0), new CWPoint(0, 1));
		mapName = MyLocale.getMsg(4300, "empty 1 Pixel = ") + scalei
				+ MyLocale.getMsg(4301, "meters");
		double meters2deg = 1 / (1000 * (new CWPoint(0, 0))
				.getDistance(new CWPoint(1, 0)));
		double pixel2deg = meters2deg * scalei;
		affine[0] = 0; // x2lat
		affine[1] = pixel2deg
				/ java.lang.Math.cos(lat * java.lang.Math.PI / 180); // x2lon
	...
	/**
	 * constructs an MapInfoObject with an associated map with 1 Pixel = scale
	 * meters, centre and width, hight in pixels
	 * 
	 * @param name
	 *            path and filename of .wfl file without the extension (it is
	 *            needed because the image will be searched in the same
	 *            directory)
	 */
	public MapInfoObject(double scalei, CWPoint center, int width, int hight,
			String name) {
		super();
		mapName = name + ".wfl";
		double meters2deg = 1 / (1000 * (new CWPoint(0, 0))
				.getDistance(new CWPoint(1, 0)));
		double pixel2deg = meters2deg * scalei;
		double pixel2deghorizontal = pixel2deg
				/ java.lang.Math.cos(center.latDec * java.lang.Math.PI / 180);
		affine[0] = 0; // x2lat
	...
	/**
	 * Method to evaluate ground control points (georeferenced points) and
	 * identify the parameters for the affine transformation
	 * 
	 * @throws IllegalArgumentException
	...
	public void evalGCP(ewe.util.Vector GCPs, int imageWidth, int imageHeight,
		...
		if (GCPs.size() < 3)
			throw new IllegalArgumentException(MyLocale.getMsg(4304,
					"not enough points to calibrate the map"));
	...
	/**
	 * calculates centre, diagonal size of the map and inverse to affine
	 * transformation
	 * 
	 * @throws ArithmeticException
	 *             when affine data is not correct, e.g. it is not possible to
	...
	private void doCalculations() throws ArithmeticException {
		try {
			...
			center.set((buttomright.latDec + topleft.latDec) / 2,
					(buttomright.lonDec + topleft.lonDec) / 2);
			sizeKm = java.lang.Math.abs((float) center.getDistance(
					buttomright.latDec, buttomright.lonDec)) * 2;
	...
	public void saveWFL(String mapsPath, String mapFileName)
		...
		towriteB
				.append(((coordTrans == 0 || coordTrans == TransformCoordinates.EPSG_WGS84) ? ""
						: Convert.toString(coordTrans) + "\n"));
	 ...
	 * @param diffX
	 *            shift of map in pixels (if the map was cropped while zooming)
	 *            in the not zoomed image
	 * @param diffY
	 */
	...
	/**
	 * Method to calculate bitmap x,y of the current map using lat and lon
	 * target coordinates. There ist no garanty that the returned coordinates
	 * are inside of the map. They can be negative.
	 * 
	 * @param lat
	 * @param lon
	 */
	...
	/**
	 * Get the prefix used for easy and fast finding of the best map The filname
	 * of the .wfl and respective image should start with this prefix in order
	 * to make finding the best map much faster
	 * 
	 * @return
	 */
...
/**
 * Class based on CWPoint but intended to handle bitmap x and y Used for
 * georeferencing bitmaps.
 */
