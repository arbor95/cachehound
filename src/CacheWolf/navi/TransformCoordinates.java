 ...
 * Helmert Transformation programmed according to
 * http://www.geoclub.de/files/GK_nach_GPS.xls and
 * http://www.geoclub.de/files/GPS_nach_GK.xls The only difference to the
 * excel-model is that shifting is done before rotation this makes calculations
 * easier, without changing the output.
 * 
 * For verification data see: *
 * http://crs.bkg.bund.de/crseu/crs/descrtrans/BeTA/BETA2007testdaten.csv *
 * http://www.lverma.nrw.de/produkte/raumbezug/koordinatentransformation/
 * Koordinatentransformation.htm Now, that this is completed: there is a much
...
public class TransformCoordinates {
	...
	private static final Ellipsoid BESSEL = new Ellipsoid(6377397.155,
			6356078.962, true);
	public static final Ellipsoid WGS84 = new Ellipsoid(6378137.000,
			6356752.314, true);
	public static final Ellipsoid HAYFORD1909 = new Ellipsoid(6378388, 297,
			false);

	// taken from http://crs.bkg.bund.de/crs-eu/ click on "national CRS" ->
	// germany -> DE_DHDN / GK_3 -> DE_DHDN (North) to ETRS89
	// they are the same as http://www.geoclub.de/files/GK_nach_GPS.xls
	// "Parametersatz 4 = Deutschland Nord"
	private static final TransformParameters GK_NORD_GERMANY_TO_WGS84 = new TransformParameters(
			590.5, 69.5, 411.6, 0.796, 0.052, 3.601, 8.300);
	/**
	 * use this for nord Germany, maximum deviation sub meter, valid in the
	 * former BRD (west germany) in 52°20' N ... 55°00' N
	 */
	public static final TransformParameters GK_NORD_GERMANY = GK_NORD_GERMANY_TO_WGS84;

	// taken from http://crs.bkg.bund.de/crs-eu/ click on "national CRS" ->
	// germany -> DE_DHDN / GK_3 -> DE_DHDN (Middle) to ETRS89
	private static final TransformParameters GK_MID_GERMANY_TO_WGS84 = new TransformParameters(
			584.8, 67.0, 400.3, -0.105, -0.013, 2.378, 10.290);
	/**
	 * use this for mid-Germany, maximum deviation sub meter, valid in the
	 * former BRD (west germany) in 50°20' N ... 52°20' N
	 */
	public static final TransformParameters GK_MID_GERMANY = GK_MID_GERMANY_TO_WGS84;

	// taken from http://crs.bkg.bund.de/crs-eu/ click on "national CRS" ->
	// germany -> DE_DHDN / GK_3 -> DE_DHDN (South) to ETRS89
	private static final TransformParameters GK_SOUTH_GERMANY_TO_WGS84 = new TransformParameters(
			597.1, 71.4, 412.1, -0.894, -0.068, 1.563, 7.580);
	/**
	 * use this for south Germany, maximum deviation sub meter, valid in the
	 * former BRD (west germany) in 47°00' N ... 50°20' N
	 */
	...
	// for NRW this transform has deviations lower than 34cm.
	private static final TransformParameters GK_NRW_GERMANY_TO_WGS84 = new TransformParameters(
			566.1, 116.3, 390.1, -1.11, -0.24, 3.76, 12.6);
	...
	// taken from
	// http://www.lverma.nrw.de/produkte/druckschriften/verwaltungsvorschriften/images/gps/TrafopsNRW.pdf
	// for NRW this transform has deviations lower than 113cm.
	// these matches to http://www.geoclub.de/files/GK_nach_GPS.xls
	// "Parametersatz 3 = Deutschland 1995"
	private static final TransformParameters GK_GERMANY_1995_TO_WGS84 = new TransformParameters(
			582, 105, 414, -1.04, -0.35, +3.08, 8.3);
	/**
	 * Use this for Germany if there is no more specific available. Deviations
	 * less than 113 cm
	 */
	public static final TransformParameters GK_GERMANY_1995 = GK_GERMANY_1995_TO_WGS84;

	// taken from
	// http://www.geodatenzentrum.de/geodaten/gdz_home1.gdz_home_start?gdz_home_para1=Technische%A0Hinweise&gdz_home_para2=Technische%A0Hinweise&gdz_home_menu_nr=10&gdz_home_menu_nr2=1&gdz_home_para3=/auftrag/html/gdz_tech_geo_deu.htm&gdz_home_spr=deu&gdz_home_para0=0
	private static final TransformParameters GK_GERMANY_BKG_TO_WGS84 = new TransformParameters(
			586, 87, 409, -0.52, -0.15, 2.82, 9);
	/**
	 * Use this for Germany if there is no more specific available. Deviations
	 * unknown. Data source: Bundesamt für Kartographie und Geodäsie, taken from
	 * website on: 1-11-2007
	 */
	public static final TransformParameters GK_GERMANY_BKG = GK_GERMANY_BKG_TO_WGS84;

	// take from http://www.geoclub.de/files/GK_nach_GPS.xls
	// "Parametersatz 2 = Deutschland 2001"
	private static final TransformParameters GK_GERMANY_2001_TO_WGS84 = new TransformParameters(
			598.1, 73.7, 418.2, -0.202, -0.045, 2.455, 6.700);
	/**
	 * Use this for Germany if there is no more specific available. maximal
	 * deviations unknown
	 */
	...
	// taken from http://crs.bkg.bund.de/crs-eu/ -> italy -> ROMA40 (change the
	// sign of the rotation parameters!)
	public static final TransformParameters GB_ITALIAN_PENINSULAR_TO_WGS84 = new TransformParameters(
	...
	/*
	 * replaced by getGkRegion public static boolean isGermanGk(int epsgcode) {
	 * boolean ret = false; switch (epsgcode) { case EPSG_GK2: case EPSG_GK3:
	 * case EPSG_GK4: case EPSG_GK5: ret = true; } return ret; }
	 */

	/**
	 * 
	 * @param epsgcode
	 * @return region code as needed for GkPoint, -1 if not Gauß-Krüger or not
	 *         supported
	 */
	...
	/**
	 * This is the most abstract method: If you don't know when to use another
	 * one (if you are in need to do so, you will know), use this one.
	 * 
	 * @param gk
	 * @return
	 */
	public static CWPoint germanGkToWgs84(GkPoint gk) {
		if (gk.northing <= 6089288.064
				...
				|| (gk.getStripe() == 5
						&& gk.getGkEasting(GkPoint.GERMAN_GK) >= 5211904.597 && gk
						.getGkEasting(GkPoint.GERMAN_GK) <= 5466056.603))
			return gkToWgs84(gk, BESSEL, GK_GERMANY_2001, 1);
		if (gk.northing <= 6097247.910 && gk.northing >= 5800464.725)
			return gkToWgs84(gk, BESSEL, GK_NORD_GERMANY, 1);
		if (gk.northing <= 5800464.725 && gk.northing >= 5577963.555)
			return gkToWgs84(gk, BESSEL, GK_MID_GERMANY, 1);
		if (gk.northing <= 5577963.555 && gk.northing >= 5207294.028)
			return gkToWgs84(gk, BESSEL, GK_SOUTH_GERMANY, 1);
		return gkToWgs84(gk, BESSEL, GK_GERMANY_2001, 1);
	}
	...
	public static CWPoint italianGkToWgs84(GkPoint gk) {
		...
		if (ITALY_SICILIA_GK.isInBound(gk.toTrackPoint(GkPoint.ITALIAN_GB)))
			...
			return gkToWgs84(gk, HAYFORD1909, GB_ITALIAN_PENINSULAR_TO_WGS84,
					0.9996);
	}
	...
	public static CWPoint GkToWgs84(GkPoint gk, int region) {
		...
		case GkPoint.GERMAN_GK:
			return germanGkToWgs84(gk);
		case GkPoint.ITALIAN_GB:
			return italianGkToWgs84(gk);
		}
		throw new IllegalArgumentException("GkToWgs84: region: " + region
				+ " not supported");
	}

	/**
	 * This is the most abstract method: If you don't know when to use another
	 * one (if you are in need to do so, you will know), use this one. This
	 * routine chooses automatically the best known transformation parameters.
	 * Currently the maximal deviation is 1m for the former BRD and 1.13m for
	 * the former GDR It also chooses automatically the correct stripe
	 * 
	 * @param gk
	 * @return
	 */
	...
	public static GkPoint wgs84ToGk(TrackPoint ll, int region) {
		...
		case GkPoint.GERMAN_GK:
			return wgs84ToGk(ll, BESSEL, getGermanGkTransformParameters(ll),
					-1, 3, 0, 1);
		case GkPoint.ITALIAN_GB:
			return wgs84ToGk(ll, HAYFORD1909,
					getItalianGkTransformParameters(ll), -1, 6, 3, 0.9996);
		default:
			throw new IllegalArgumentException(
					"wgs84ToGk(CWPoint, int): region: " + region
							+ "not supported");
		}
	}
	...
	/**
	 * This function returns the position in the list of the given epsg code
	 * list which corresondes to the stripe used in Gauß-Krüger Point gk
	 * 
	 * @param epsgcodes
	 *            list of epsgcodes
	 * @param gk
	...
	public static int whichEpsg(int[] epsgcodes, GkPoint gk) {
		...
		for (i = 0; i < epsgcodes.length; i++) {
			if (getGkStripeEpsg(epsgcodes[i]) == stripe)
				break;
		}
		if (i >= epsgcodes.length)
			return -1;
		return i;
	}

	/**
	 * Call this routine to convert from wgs84 into German
	 * Gauß-Krüger-Coordinates using the Gauß-Krüger Projection and the
	 * ellipsoid forgk If you want the Gauß-Krüger-Coordinates in a certain
	 * stripe, provide the stripe and stripe width, otherwise set stripe to -1,
	 * then the stripe will be automatically determined
	 * 
	 * @param ll
	 * @param Gauß
	 *            -Krüger-to-WGS84 transformation parameters, they will be
	 *            automatically inverted
	 * @param stripe
	 *            stripe to force to, otherwise -1 will determine the stripe
	 *            automatically
	 * @return
	 */
	...
	public static GkPoint wgs84ToGk(TrackPoint ll, Ellipsoid forgk,
		...
		// ewe.sys.Vm.debug("wgs84-ll: " + new
		// CWPoint(ll).toString(CWPoint.DMS));
		// ewe.sys.Vm.debug("gkll: " + gkll.toString(CWPoint.DMS));
		if (stripe == -1)
			return projectLatlon2GkStripeauto(gkll, forgk, stripewidth,
					degreeOfStripe0, scale);
		else
			return projectLatlon2GK(gkll, forgk, stripewidth, stripe,
					degreeOfStripe0, scale);
	}

	/**
	 * Call this method to convert any Gauß-Krüger coordinates into wgs84.
	 * 
	 * @param gk
	 *            point to convert
	 * @param GK2WGS84
	 *            Gauß-Krüger-to-WGS84 transformation parameters
	 * @return
	 */
	public static CWPoint gkToWgs84(GkPoint gk, Ellipsoid gkon,
			TransformParameters gk2wgs84, double scale) {
	...
	/**
	 * this routine gives the correct Gauß-Krüger coordinates in the stripe
	 * specified by EPSG-Code
	 * 
	 * @param wgs84
	 * @param epsgcode
	 * @return
	...
	public static GkPoint wgs84ToGaussKrueger(TrackPoint wgs84, int epsgcode)
		...
		case GkPoint.GERMAN_GK:
			return wgs84ToGk(wgs84, BESSEL,
					getGermanGkTransformParameters(wgs84),
					getGkStripeEpsg(epsgcode), 3, 0, 1);
		case GkPoint.ITALIAN_GB:
			return wgs84ToGk(wgs84, HAYFORD1909,
					getItalianGkTransformParameters(wgs84),
					getGkStripeEpsg(epsgcode), 6, 3, 0.9996);
		}
		throw new IllegalArgumentException("wgs84ToGaussKrueger: epsg-code: "
				+ epsgcode + "not supported");
	}
	...
	private static int getGkStripeEpsg(int epsgcode) {
		...
		case EPSG_ITALIAN_GB_EW2:
			...
			break;
		default:
			throw new IllegalArgumentException("wgs84ToGermanGk: epsgcode: "
					+ epsgcode
					+ MyLocale.getMsg(4900,
							" is not a german Gauss-Krueger coordinate"));
		}
		return stripe;
	}
	...
	private static XyzCoordinates transform(XyzCoordinates from,
	...
	private static CWPoint xyz2Latlon(XyzCoordinates from, Ellipsoid ellipsoid) {
		...
		double B = Math.atan((from.z + e2 * Math.pow(ellipsoid.a, 2)
				/ ellipsoid.b * Math.pow(Math.sin(T), 3))
				/ (s - e2 * ellipsoid.a * Math.pow(Math.cos(T), 3)));
	...
	private static GkPoint projectLatlon2GkStripeauto(CWPoint latlon,
			Ellipsoid ellipsoid, int stripewidth, float degreeOfStripe0,
			double scale) {
		if (!latlon.isValid())
			throw new IllegalArgumentException(
		...
		for (stripe = 0; stripe <= 360; stripe += stripewidth) {
			if (Math.abs(lonDec - stripe) <= ((float) stripewidth) / 2)
				break;
		}
		return projectLatlon2GK(latlon, ellipsoid, stripewidth, stripe
				/ stripewidth, degreeOfStripe0, scale);
	}
	 ...
	 * @return
	 */
	private static GkPoint projectLatlon2GK(CWPoint latlon,
			Ellipsoid ellipsoid, int stripewidth, int stripe,
		...
		double n2 = (ellipsoid.a + ellipsoid.b) / 2
				* (1 + Math.pow(n1, 2) / 4 + Math.pow(n1, 4) / 64);
		double n3 = n1 * -3 / 2 + Math.pow(n1, 3) * 9 / 16 - Math.pow(n1, 5)
				* 3 / 32;
		...
		double arclength = n2
				* (B + n3 * Math.sin(B * 2) + n4 * Math.sin(B * 4) + n5
						* Math.sin(B * 6) + n6 * Math.sin(B * 8));

		double h1 = t / 2 * N * Math.pow(Math.cos(B), 2) * l * l;
		double h2 = t / 24 * N * Math.pow(Math.cos(B), 4)
				* (5 - t * t + 9 * nue * nue + 4 * Math.pow(nue, 4))
				* Math.pow(l, 4);
	 ...
	 * @param ellipsoid
	 * @param stripewidth
	 *            width in degree of the stripe of the Gauß-Krüger-System (3
	 *            degreee usually used in Gauß-Krüger, 6 degree usually in UTM)
	 * @return
	 */
	private static CWPoint gk2LatLon(GkPoint gkp, Ellipsoid ellipsoid,
		...
		double e2 = (ellipsoid.a * ellipsoid.a - ellipsoid.b * ellipsoid.b)
				/ (ellipsoid.a * ellipsoid.a);
		// note: n1-n6 are similiar to the n1-n6 in projectLatlon2GK, but some
		// term have different factors
		double n1 = (ellipsoid.a - ellipsoid.b) / (ellipsoid.a + ellipsoid.b);
		double n2 = (ellipsoid.a + ellipsoid.b) / 2
				* (1 + Math.pow(n1, 2) / 4 + Math.pow(n1, 4) / 64);
		double n3 = n1 * 3 / 2 - Math.pow(n1, 3) * 27 / 32 + Math.pow(n1, 5)
				* 269 / 32;
		...
		double Bf = B0 + n3 * Math.sin(B0 * 2) + n4 * Math.sin(B0 * 4) + n5
				* Math.sin(B0 * 6) + n6 * Math.sin(B0 * 8);

		double Nf = ellipsoid.a / Math.sqrt(1 - e2 * Math.pow(Math.sin(Bf), 2));
		double nuef = Math.sqrt(ellipsoid.a * ellipsoid.a / ellipsoid.b
				/ ellipsoid.b * e2 * Math.pow(Math.cos(Bf), 2));
		...
		double la2 = tf
				/ 24
				/ Math.pow(Nf, 4)
				* (5 + 3 * tf * tf + 6 * nuef * nuef - 6 * tf * tf * nuef
						* nuef - 4 * Math.pow(nuef, 4) - 9 * tf * tf
						* Math.pow(nuef, 4)) * Math.pow(y, 4);
		// these deal with less than the overall calculation precision: double
		// la3 = tf /720 / Math.pow(Nf, 6) * (-61 - 90*tf*tf - 45*Math.pow(tf,4)
		// - 107*nuef*nuef + 162*tf*tf * Math.pow(nuef, 2) +
		// 45*Math.pow(tf,4)*tf*Math.pow(nuef, 2)) * Math.pow(y, 6);
		// these deal with less than the overall calculation precision: double
		// la4 = tf /40320 / Math.pow(Nf, 8) * (1385+3663*tf*tf -
		// 4095*Math.pow(tf,4) + 1575*Math.pow(nuef, 6)) * Math.pow(y, 8);
...
class TransformParameters {
	 ...
	 *            shift in meter
	 * @param exi
	 *            rotation in seconds (change the sign of the values from
	 *            http://crs.bkg.bund.de/crs-eu/ )
	 * @param si
	 *            deviation of scale multiplied by 10^6
	 * @param addinverted
	 */
	public TransformParameters(double dxi, double dyi, double dzi, double exi,
			double eyi, double ezi, double si) {
		set(dxi, dyi, dzi, exi, eyi, ezi, si, true);
	}

	protected void set(double dxi, double dyi, double dzi, double exi,
			double eyi, double ezi, double si, boolean addinverted) {
		dx = dxi;
		dy = dyi;
		dz = dzi;
