...
public class Area {
	...
	// if(affine[4] >= lati && lati >= lowlat && affine[5] <= loni && loni <=
	// lowlon) isInBound = true;

	public boolean isInBound(double lat, double lon) {
		if (topleft.latDec >= lat && topleft.lonDec <= lon
				&& buttomright.latDec <= lat && buttomright.lonDec >= lon)
			return true;
		else
			return false;
	}
	...
	public boolean equals(Area a) {
		if (java.lang.Math.abs(topleft.latDec - a.topleft.latDec) < edgeTolerance
				...
				&& java.lang.Math
						.abs(buttomright.latDec - a.buttomright.latDec) < edgeTolerance
				&& java.lang.Math
						.abs(buttomright.lonDec - a.buttomright.lonDec) < edgeTolerance)
			return true;
		else
			return false;
	}
	...
	public CWPoint getCenter() {
		return new CWPoint((topleft.latDec + buttomright.latDec) / 2,
				(topleft.lonDec + buttomright.lonDec) / 2);
	}
}
