...
public class GkPoint {
	...
	public GkPoint(double e, double n, int stripewidthi,
			float degreeOfStripeZero) {
		stripe = (int) Math.floor(e / 1000000);
		set(e - 1000000 * stripe - 500000, n, stripe, stripewidthi,
				degreeOfStripeZero);
	}

	/**
	 * use this to set normal german Gauß-Krüger coordinates (they contain the
	 * stripe numer in the easting value and have a stripe with of 3 degrees)
	 * 
	 * @param e
	 * @param n
	 */
	public GkPoint(double e, double n, int region) {
		...
		default:
			throw new IllegalArgumentException(
					"GkPoint (double, double, int): region: " + region
							+ " not supported");
		}
	}
	...
	/**
	 * @param e
	 *            in meters from center of stripe, it may not contain the
	 *            stripenumber
	 */
	public void set(double e, double n, int stripei, int stripewidthi,
			float lenthOfStripeZero_) {
		stripe = stripei;
	...
	/**
	 * This will give you the normal Gauß-Krüger easting value (that means
	 * including the stripe number)
	 * 
	 * @return
	 */
	 ...
	 * 
	 * @param region
	 *            international telephone area code * 100
	 * @return
	...
	public String toString(int decimalplaces, String prefix, String seperator,
		...
		return prefix + e.toString().replace(',', '.') + seperator
				+ n.toString().replace(',', '.');
	}
