...
/**
 * This class displays a user interface to select maps that should be or are
 * already calibrated class id = 4100
 */
public class SelectMap extends Form {
	...
	public SelectMap() {
		...
		this.addLast(new mLabel(MyLocale.getMsg(4103, "Non Calibrated Maps")),
				CellConstants.STRETCH, CellConstants.FILL);
		this.addLast(bot, CellConstants.STRETCH, CellConstants.FILL);
		this.addNext(mBCancel = new mButton(MyLocale.getMsg(4104, "Cancel")),
				CellConstants.STRETCH, CellConstants.FILL);
		this.addLast(mBLoad = new mButton(MyLocale.getMsg(4105, "Open")),
				CellConstants.STRETCH, CellConstants.FILL);
		...
		try {
			...
			dateien = files.listMultiple("*.png,*.jpg,*.gif,*.bmp",
