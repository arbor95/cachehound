...
public class UrlFetcher {
	public static String fetchString(String address) throws IOException {
		...
		CharArray c_data = codec.decodeText(daten.data, 0, daten.length, true,
				null);
	 ...
	 * @throws IOException
	 */
	public static ByteArray fetchByteArray(String url, CharArray realurl)
			throws IOException {
		...
		do { // allow max 5 redirections (http 302 location)
			if (realurl != null)
				realurl.copyFrom(new String(urltmp));
			i++;
		...
		if (i > maxRedirections)
			throw new IOException(
					"too many http redirections while trying to fetch: " + url
							+ " only " + maxRedirections + " are allowed");
	...
	/**
	 * @param url
	 * @return true, if the string seems to be already URL encoded (that is, it
	 *         contains only url-allowd chars), false otherwise
	 */
