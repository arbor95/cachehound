...
public class SearchBox extends InputBox {
	...
	/**
	 * Displays the search Box and returns the String value entered, if OK is
	 * pressed, otherwise the value is null.
	 * 
	 * @param initialValue
	 *            Initial value to display in the search box
	 * @param checkUseNoteDesc
	 ...
	 */
	public String input(String initialValue, boolean checkUseNoteDesc,
			boolean checkUseLogs, int pWidth) {
		return this.input(null, initialValue, checkUseNoteDesc, checkUseLogs,
				pWidth);
	}

	protected String input(Frame pParent, String initialValue,
			boolean checkUseNoteDesc, boolean checkUseLogs, int pWidth) {
		String result;
		buildingForm = true;
	...
	public Control addLast(Control c) {
		// This method is a dirty hack, because in InputBox every thing, from
		// creation of the
		// controls to displaying it and returning the return value is done at
		// once.
		// To be able to add other controls, I have to enhance the
		// addLast(Control) method - not
		// nice but it works.
		Control result;
		if (!buildingForm) {
	...
	/**
	 * Called before creating the input box. Additional controls may be added
	 * here.
	 */
	...
	/**
	 * Called after creating the input box. Additional controls may be added
	 * here.
	 */
	private void addControlsAfterInput() {
		if (useNoteDesc != null) {
			this.addLast(useNoteDesc, CellConstants.DONTSTRETCH,
					(CellConstants.DONTFILL | CellConstants.WEST));
		}
		if (useLogs != null) {
			this.addLast(useLogs, CellConstants.DONTSTRETCH,
					(CellConstants.DONTFILL | CellConstants.WEST));
		}
	}
