...
public class Rebuild {
	...
	public void rebuild() {
			...
			for (i = 0; i < xmlFiles.length; i++) {
				if (xmlFiles[i] != null) {
					...
					if (details != null) { // In older Versions of CW the
						...
						Global.getPref().log(
								"File " + xmlFiles[i] + " not in index.xml");
				}
				if (pbf.isClosed)
					break;
			}
			(new MessageBox(MyLocale.getMsg(327, "Information"), MyLocale
					.getMsg(210, "Caches nicht in index.xml: ")
					+ orphans
					+ MyLocale.getMsg(211, "\nDavon hinzugefügt: ")
					+ nAdded, FormBase.OKB)).execute();
		...
		if (orphans != nAdded
				&& (new MessageBox(
						...
						MyLocale
								.getMsg(212,
										"Delete all .xml files not in index.xml and associated pictures"),
			...
			for (i = 0; i < xmlFiles.length; i++) {
				if (xmlFiles[i] != null) {
					h.progress = ((float) nDeleted++)
							/ (float) (orphans - nAdded);
					h.changed();
					int dotPos = xmlFiles[i].indexOf('.');
					if (dotPos > 0) {
						// This may appear when there are directories in the
						// profile
						String wayPoint = xmlFiles[i].substring(0, dotPos);
						dm.deleteCacheFiles(wayPoint, prof.getDataDir());
					}
				}
			}
		}
	...
	private String getCacheDetails(String xmlFile) {
		try {
			...
			// Check that we have not accidentally listed another xml file in
			// the directory
			if (text.indexOf("<CACHEDETAILS>") < 0
					|| (start = text.indexOf("<CACHE ")) < 0)
				return null;
			if ((vstart = text.indexOf("<VERSION value = \"")) >= 0) {
				cacheXmlVersion = Integer.valueOf(
						text.substring(vstart + 18, text.indexOf("\"",
								vstart + 18))).intValue();
			}
