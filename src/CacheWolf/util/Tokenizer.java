...
/**
 * Class to tokenise (break up) the code into single tokens, so the parser my do
 * its job.
 * 
 * @see Parser
 */
public class Tokenizer {
	...
	/**
	 * Normally only a semicolon (;) can be used to separate commands. If this
	 * variable is set to true, newlines also terminate a command. If a newline
	 * is preceded with a backslash (=line continuation character), the newline
	 * does not terminate the command even if this variable is true.
	 */
	...
	public Vector TokenStack = new Vector();
	/** position of token */
	...
	private void err(String str) throws Exception {
		messageStack.add(MyLocale.getMsg(1700, "Error on line: ") + currentLine
				+ "  " + MyLocale.getMsg(1701, " position: ") + currentPos);
		messageStack.add(str);
		if (Global.mainTab.solverP.mText.setSelectionRange(0, currentLine - 1,
				currentPos, currentLine - 1))
	...
	/**
	 * Create a new token object and remember the place where it started. String
	 * tokens could span several lines, so we need to remember the starting line
	 * and position.
	 */
	...
	private void streamString() throws Exception {
		...
		while (getChar()) { // collect chars until next "
			if (look == '"') {
				...
				currentStream += look;
			// Need to count newlines inside a string spanning multiple lines so
			// that we don't loose track
			if (look == '\n') {
				currentLine++;
				currentPos = 0;
			}
