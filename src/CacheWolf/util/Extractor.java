...
public class Extractor {
	...
	/**
	 * Create an extractor. sTxt = The string to search through.<br>
	 ...
	 * only = if false the string returned will inlcude st and e; if true it
	 * will not include st and e.
	 * 
	 */
	 ...
	 * start (st) and end string (e). if end is not found the string is returned
	 * til it's end.
	 */
	public String findNext() {
		if (searchText == null)
			return new String(); // maby null should
		...
		if (idxStart > -1) {
			if (betweenonly == false) {
				if (idxEnd + end.length() >= searchText.length())
					...
					tst = searchText.substring(idxStart, idxEnd + end.length());
			} else {
