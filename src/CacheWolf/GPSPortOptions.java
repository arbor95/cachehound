...
class GpsdThread extends mThread {
	...
	public GpsdThread(TextDisplay td) throws IOException {
		try {
			gpsdSocket = new Socket(Global.getPref().gpsdHost,
					Global.getPref().gpsdPort);
		} catch (IOException e) {
...
public class GPSPortOptions extends SerialPortOptions {
	...
	public Editor getEditor() {
		...
		// Because this is from the general SerialPortOptions class, maybe not
		// all of the code
		// must be necessary.
		...
		ed.addField(
				cp.addNext(new mComboBox()).setCell(CellConstants.HSTRETCH),
				"portName");
		btnUpdatePortList = new mButton(MyLocale.getMsg(7101, "Update Ports$u"));
		ed.addField(cp.addLast(btnUpdatePortList).setCell(
				CellConstants.DONTSTRETCH), "update");
		is.add(cp, "Port:$p");
		mComboBox cb = new mComboBox();
		is.add(ed.addField(cb, "baudRate"), MyLocale.getMsg(7102, "Baud:$b"));
		cb.choice.addItems(ewe.util.mString
				.split("110|300|1200|2400|4800|9600|19200|38400|57600|115200"));
		//
		...
		ed.addField(ed.addLast(btnTest.setCell(CellConstants.DONTSTRETCH)),
				"test");
		txtOutput = new TextDisplay();
		ScrollBarPanel sbp = new MyScrollBarPanel(txtOutput);
		sbp.setOptions(ScrollablePanel.AlwaysShowVerticalScrollers
				| ScrollablePanel.AlwaysShowHorizontalScrollers);
		ed.addField(ed.addLast(sbp), "out");

		forwardGpsChkB = new mCheckBox("");
		ed.addField(ed.addNext(forwardGpsChkB, CellConstants.DONTSTRETCH,
				(CellConstants.EAST | CellConstants.DONTFILL)),
				"forwardGpsChkB");
		labelForwardHost = new mLabel(MyLocale.getMsg(7105,
				"Forward GPS data to host"));
		ed.addField(ed.addNext(labelForwardHost, CellConstants.DONTSTRETCH,
				(CellConstants.WEST | CellConstants.DONTFILL)),
				"labelForwardIP");
		...
		inputBoxForwardHost
				.setToolTip(MyLocale
						.getMsg(
								7106,
								"All data from GPS will be sent to TCP-port 23\n and can be redirected there to a serial port\n by HW Virtual Serial Port"));
		ed.addField(ed.addLast(inputBoxForwardHost, 0,
				(CellConstants.WEST | CellConstants.HFILL)), "tcpForwardHost");

		gpsdChkB = new mCheckBox("");
		ed.addField(ed.addNext(gpsdChkB, CellConstants.DONTSTRETCH,
				(CellConstants.EAST | CellConstants.DONTFILL)), "gpsdChkB");
		labelGpsdHost = new mLabel(MyLocale.getMsg(7121,
				"Receive GPS from gpsd host"));
		ed.addField(ed.addNext(labelGpsdHost, CellConstants.DONTSTRETCH,
				(CellConstants.WEST | CellConstants.DONTFILL)), "labelGpsd");
		inputBoxGpsdHost = new mInput("GpsdHost");
		inputBoxGpsdHost.setPromptControl(labelGpsdHost);
		inputBoxGpsdHost
				.setToolTip(MyLocale
						.getMsg(7122,
								"GPS data will be received from a gpsd server, not from a serial port"));
		ed.addField(ed.addLast(inputBoxGpsdHost, 0,
				(CellConstants.WEST | CellConstants.HFILL)), "GpsdHost");

		logGpsChkB = new mCheckBox("");
		ed.addField(ed.addNext(logGpsChkB, CellConstants.DONTSTRETCH,
				(CellConstants.EAST | CellConstants.DONTFILL)), "logGpsChkB");
		labelLogTimer = new mLabel(MyLocale.getMsg(7107,
				"Interval in sec for logging"));
		ed
				.addField(ed.addNext(labelLogTimer, CellConstants.DONTSTRETCH,
						(CellConstants.WEST | CellConstants.DONTFILL)),
						"labelLogTimer");
		inputBoxLogTimer = new mInput("GPSLogTimer");
		inputBoxLogTimer.setPromptControl(labelLogTimer);
		ed.addField(ed.addLast(inputBoxLogTimer, 0,
				(CellConstants.WEST | CellConstants.HFILL)), "GPSLogTimer");
			...
			if (scanRunning == false) {
				...
				new mThread() {
					public void run() {
						...
						btnTest.repaintNow();
						btnScan.setText(Gui.getTextFrom(MyLocale.getMsg(7119,
								"Stop")));
						btnScan.repaintNow();
						...
						if (ports == null) {
							...
							if (i >= ports.length)
								txtOutput.appendText(MyLocale.getMsg(7110,
										"GPS not found\n"), true);
						}
						fin();
					}
					...
					private void fin() {
						...
						if (btnScan != null) {
							btnScan.setText(Gui.getTextFrom(MyLocale.getMsg(
									7103, "Scan$u")));
							btnScan.repaintNow();
						}
					}
		...
		if (field.equals("test")) {
			if (!gpsRunning) {
				...
				if (Global.getPref().useGPSD) {
					txtOutput.setText(MyLocale.getMsg(99999,
							"Displaying data from gpsd directly:\n"));
					try {
						btnScan.set(ControlConstants.Disabled, true);
						btnScan.repaintNow();
						gpsdThread = new GpsdThread(txtOutput);
						gpsdThread.start();
						btnTest.setText(Gui.getTextFrom(MyLocale.getMsg(7118,
								"Stop")));
						gpsRunning = true;
					} catch (IOException e) {
						(new MessageBox(
					...
					txtOutput.setText(MyLocale.getMsg(7117,
							"Displaying data from serial port directly:\n"));
					try {
						btnScan.set(ControlConstants.Disabled, true);
						btnScan.repaintNow();
						...
						btnScan.repaintNow();
						txtOutput.appendText(MyLocale.getMsg(7108,
				...
				if (gpsdThread != null)
					gpsdThread.stop();
				btnTest.setText(Gui
						.getTextFrom(MyLocale.getMsg(7104, "Test$t")));
				gpsRunning = false;
				btnScan.set(ControlConstants.Disabled, false);
				btnScan.repaintNow();
			}
	...
	private boolean testPort(String port, int baud) {
		...
		// if (!gpsPort.isOpen()) txtOutput.appendText(MyLocale.getMsg(7108,
		// "Failed (2) to open serial port: ") + this.portName + "\n", true);

		// try to read some data
		now = new Time().getTime();
		txtOutput.appendText(MyLocale.getMsg(7111, "Trying ") + port
				+ MyLocale.getMsg(7112, " at ") + baud + " Baud\n", true);
		gpsPort.start();
		boolean gpsfound = false;
		boolean gotdata = false;
		while ((new Time().getTime() - now) < 3000 && !gpsfound) {
			// gpsLen = gpsPort.lastgot.length(); // nonBlockingRead(gpsBuff,0,
			// gpsBuff.length);
		...
		if (gpsfound)
			...
			if (gotdata)
				txtOutput.appendText(MyLocale.getMsg(7115,
						" - No GPS data tag found\n"), true);
			else
				txtOutput.appendText(MyLocale.getMsg(7116,
						" - No data received\n"), true);
		}
		// catch (IOException io) {
		// txtOutput.appendText("error closing serial port", true); }
		return gpsfound;
	}
