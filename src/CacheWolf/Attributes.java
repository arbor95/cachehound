...
public class Attributes {
	...
	/**
	 * The number of attributes for this cache (=number of array elements in use
	 * in attribs)
	 */
	private int count = 0;
	/**
	 * The attributes are represented as a byte array with each element encoding
	 * one attribute
	 */
		...
		if (count < attribs.length) {
			if (!attributeName.equalsIgnoreCase("attribute-blank.gif")) {
				...
				if (attributeName.endsWith("-yes.gif"))
					attributesYes |= (1l << ((long) (Math
							.ceil(attr.getAttrNr() / 2.0) - 1.0)));
				else
					attributesNo |= (1l << ((long) (Math
							.ceil(attr.getAttrNr() / 2.0) - 1.0)));
			}
		}
	}
	 ...
	 * 
	 * @param i
	 *            The number of the attribute to retrieve
	 * @return The name of the attribute (e.g. horses-no.gif)
	 */
	public String getName(int i) {
		if (i >= 0 && i < count)
			return attribs[i].getImageName();
		return "error.gif";
	}
	 ...
	 * 
	 * @param i
	 *            The number of the attribute for which the image is to be
	 *            retrieved
	 * @return The mImage of the attribute
	 */
	public mImage getImage(int i) {
		if (i >= 0 && i < count)
			return attribs[i].getImage();
	 ...
	 * 
	 * @param i
	 *            The number of the attribute for which the infotext is to be
	 *            retrieved
	 * @return The infotext of the attribute
	 */
	public String getInfo(int i) {
		if (i >= 0 && i < count)
			return attribs[i].getInfo();
		return "No info";
	}
}
