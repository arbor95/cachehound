...
public class DateTimeChooser extends Editor {
	...
	public CellPanel addTopSection(CellPanel addTo, Control cp) {
		...
		b.image = new DrawnIcon(DrawnIcon.CROSS, IconSize, IconSize, new Color(
				0x80, 0, 0));
		...
		b.image = new DrawnIcon(DrawnIcon.TICK, IconSize, IconSize, new Color(
				0, 0x80, 0));
	...
	public DateTimeChooser(Locale l) {
		...
		addTable(dayChooser = new TableControl(), new dayChooserTableModel(
				locale), "day");
		addTable(monthChooser = new TableControl(), new monthChooserTableModel(
				locale), "monthName");
		addTable(yearChooser = new TableControl(), new yearChooserTableModel(),
				"year");
		addTable(timeChooser = new TableControl(), new timeChooserTableModel(),
				"time");

		// the following is already done in addTopSection?
		/*
		 * mButton b = new mButton(); //b.borderStyle =
		 * BDR_OUTLINE|BF_LEFT|BF_TOP|BF_RIGHT|BF_SQUARE; b.image = new
		 * DrawnIcon(DrawnIcon.CROSS,10,10,new Color(0x80,0,0));
		 * addField(b,"reject"); b.image = new
		 * DrawnIcon(DrawnIcon.CROSS,10,10,new Color(0x80,0,0)); b = new
		 * mButton(); b.image = new DrawnIcon(DrawnIcon.TICK,10,10,new
		 * Color(0,0x80,0)); addField(b,"accept");
		 */
		newDate();
	...
	public void onControlEvent(ControlEvent ev) {
		if (ev instanceof TableEvent && ev.type == TableEvent.CELL_CLICKED) {
			if (ev.target == dayChooser) {
				...
				timeChooserTableModel tcm = (timeChooserTableModel) timeChooser
						.getTableModel();
				int newHour = tcm.getHourFor(((TableEvent) ev).row,
						((TableEvent) ev).col);
				if (newHour > -1)
					hour = newHour;
				int newMinute = tcm.getMinuteFor(((TableEvent) ev).row,
						((TableEvent) ev).col);
				if (newMinute > -1)
					minute = newMinute;
				time = MyLocale.formatLong(hour, "00") + ":"
	...
	public void setDate(Time t) {
		...
		if (!t.isValid())
			t = new Time();
		Time.toString(t, t instanceof TimeOfDay ? t.getFormat() : locale
				.getString(Locale.SHORT_DATE_FORMAT, 0, 0), locale);
		day = t.day;
		month = t.month;
		year = t.year;
		hour = t.hour;
	...
	public void newDate() {
		...
		dayChooserTableModel dcm = (dayChooserTableModel) dayChooser
				.getTableModel();
		dcm.set(day, month, year);
		monthChooserTableModel mcm = (monthChooserTableModel) monthChooser
				.getTableModel();
...
class monthChooserTableModel extends InputPanelTableModel {
	...
	public TableCellAttributes getCellAttributes(int row, int col,
		...
		ta.borderStyle = fixBorder(UIConstants.BDR_OUTLINE
				| UIConstants.BF_BOTTOM | UIConstants.BF_RIGHT, row, col, true);
...
class yearChooserTableModel extends InputPanelTableModel {
	...
	String[] all = mString
			.split("19xx|7|8|9|20xx|4|5|6|21xx|1|2|3|18xx|0|C+|C-");
