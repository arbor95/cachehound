...
public class TritonGPXExporter extends Exporter
{
  ...
  public String record(CacheHolder ch, String lat, String lon) {
    ...
    try
    {
      ...
      String tim = (ch.getDateHidden().length() > 0) ? ch.getDateHidden() : "2000-01-01";
      strBuf.append("    <time>").append(tim.toString()).append("T00:00:00.0000000-07:00</time>\r\n");
      if (ch.isAddiWpt())
        strBuf.append("    <name>").append(ch.mainCache.getWayPoint()).append(" - ").append(ch.getWayPoint()).append("</name>\r\n");
      else {
...
//        strBuf.append("      <groundspeak:terrain>").append(ch.terrain.replace(',', '.')).append("</groundspeak:terrain>\r\n");
		...
		strBuf.append("      <groundspeak:difficulty>").append(diff).append("</groundspeak:difficulty>\r\n");
		String terr = CacheTerrDiff.shortDT(ch.getTerrain());
		strBuf.append("      <groundspeak:terrain>").append(terr).append("</groundspeak:terrain>\r\n");
        
        strBuf.append("      <groundspeak:country/>\n");
        strBuf.append("      <groundspeak:state>Nil</groundspeak:state>\n");
        strBuf.append("      <groundspeak:short_description html=\"false\">").append(ch.getWayPoint()).append(" - ").append(CacheSize.cw2ExportString(ch.getCacheSize())).append(" D:").append(diff).append("/T:").append(terr).append("</groundspeak:short_description>\r\n");
        strBuf.append("      <groundspeak:long_description html=\"false\">\r\n");

        strBuf.append("      ").append(SafeXML.removeHtml(SafeXML.strxmldecode(chdetail.LongDescription))).append("\r\n");

        if (!(chdetail.Hints.length() == 0 ))
            strBuf.append(" ######HINT!####### \r\n").append(SafeXML.removeHtml(SafeXML.strxmldecode(Common.rot13(chdetail.Hints))));
        ...
        for (int i = 0; i < logCount; ++i)
        {
          ...
          else if (chdetail.CacheLogs.getLog(i).getIcon().indexOf("sad") >= 0)
          {
            strBuf.append("[DNF]");
          }
          else
          ...
          strBuf.append("").append("from:\"").append(SafeXML.strxmldecode(chdetail.CacheLogs.getLog(i).getLogger())).append("\" \r\n");
          strBuf.append("").append(SafeXML.removeHtml(SafeXML.strxmldecode(chdetail.CacheLogs.getLog(i).getMessage()))).append(" \r\n");
        }

        strBuf.append("      \n</groundspeak:long_description>\r\n");

        if (chdetail.Hints.length() == 0 )
          //strBuf.append("\t  <groundspeak:encoded_hints>No " + ch.LatLon.replace(" ", "") + "</groundspeak:encoded_hints>\r\n");
        	strBuf.append("\t  <groundspeak:encoded_hints>No " + ch.LatLon + "</groundspeak:encoded_hints>\r\n");
        else {
          //strBuf.append("\t  <groundspeak:encoded_hints>Yes " + ch.LatLon.replace(" ", "") + "</groundspeak:encoded_hints>\r\n");
        	strBuf.append("\t  <groundspeak:encoded_hints>Yes " + ch.LatLon + "</groundspeak:encoded_hints>\r\n");
        }
        ...
        strBuf.append("    <cmt>").append(SafeXML.cleanGPX(chdetail.LongDescription)).append("</cmt>\r\n");
  ...
  public String spoiler2GPX(CacheHolderDetail ch)
  {
    ...
    for (int spoiler = 0; spoiler < ch.images.size(); ++spoiler)
    {
      ...
      GPXImages = GPXImages + "    <multimedia name=\"" + STRreplace.replace(imageName,GPXextenion, "") + "\" type=\"image/" + Type + "\">\r\n <data><![CDATA[" + GPX + "]]></data>\r\n</multimedia>\r\n";
    }
  ...
  public String addFileBase64(File file)
  {
    ...
    try
    {
      ...
      String encoded = new String(Base64Coder.encode(outStream.toByteArray()));
      return encoded;
    }
    catch (Exception e)
    {
