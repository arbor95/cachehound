...
public class GeocachingMailReader {
	...
	public GeocachingMailReader(String protocol, String host, String user, String password, String mailbox, IGCMailHandler handler) {
	...
	public void readMessages(boolean onlyNew, boolean markAsRead, boolean remove) throws MessagingException, IOException {
		int countMessages = folder.getMessageCount();
		System.out.println("Anzahl Nachrichten in '" + mailbox +"': " + countMessages);
	...
	private void readMessage(Message message, boolean markAsRead, boolean remove) throws MessagingException, IOException {
	...
	private boolean readNotify(Message message, String subject) throws IOException, MessagingException {
		...
		else if (subject.contains("retract")) { // noch nie gesehen ... aber soll ja vorkommen können ...
		...
		else if (subject.contains(" reported ") && subject.contains(" needs maintenance")) {
