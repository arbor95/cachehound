...
public class InfoBox extends Form {
	 ...
	 * clicking the "close" button. It can be used to check if a lengthy task
	 * needs to be aborted (i.e. spidering)
	 */
	...
	public InfoBox(String title, String info) {
		...
		/*
		 * this.setPreferredSize(170,50); this.title = title; msgArea = new
		 * MessageArea(""), CellConstants.STRETCH, CellConstants.FILL)
		 * this.addLast(scP = new ScrollBarPanel(msgArea));
		 * msgArea.setText(info); mB.setHotKey(0, IKeys.ACTION); mB.setHotKey(0,
		 * IKeys.ENTER); //mB.set(Control.Invisible, true); //this.addLast(mB,
	...
	public InfoBox(String title, String info, int ty, boolean autoWrap) {
		...
		case DISPLAY_ONLY:
			...
			this.addLast(msgArea.getScrollablePanel(), CellConstants.STRETCH,
					CellConstants.FILL);
			break;
		case PROGRESS_WITH_WARNINGS:
			...
			this.addLast(msgArea.getScrollablePanel(), CellConstants.HEXPAND
					| CellConstants.HGROW, CellConstants.HEXPAND
					| CellConstants.HGROW);
			warnings = new TextMessage("");
			warnings.autoWrap = autoWrap;
			this.addLast(warnings.getScrollablePanel(), CellConstants.HEXPAND
					| CellConstants.VEXPAND | CellConstants.VGROW,
					CellConstants.HEXPAND | CellConstants.VEXPAND
							| CellConstants.VGROW);
