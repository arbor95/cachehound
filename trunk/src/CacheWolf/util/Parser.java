 ...
 * 
 * New features: - Improved error handling - Strings and doubles can be freely
 * mixed as appropriate. Depending on context a conversion is performed, -
 * Variables can store strings or doubles - Global variables (starting with $)
 * are remembered across multiple calls to parser - Global variables are
 * initialised with "", local variables result in error if used before setting
 * value - IF statement added - Many new functions
 * (encode,format,goto,len,mid,count, substring,ucase,lcase,val,sval,replace,
 * reverse,project) - less typing - Function aliases - Function names can be
 * flexibly abbreviated, i.e. instead of crosstotal write cr or cross or crosst
 * ... - show no longer needed - Command terminator ; no longer compulsory (only
 * between multiple commands on same line) - New functions can easily be added -
 * Can select whether variable names are case sensitive
 * 
 * To add a new function: 1) Add its name and alias and allowed number of args
 * to array functions 2) Add a new private method in the "functions" section 3)
 * Add call to private method in executeFunction
 * 
...
public class Parser {
	...
	new fnType("abs", "abs", 2), new fnType("acos", "acos", 2),
			new fnType("asin", "asin", 2), new fnType("atan", "atan", 2),
			new fnType("bearing", "bearing", 4), new fnType("cb", "cb", 16),
			new fnType("centre", "center", 3),
			new fnType("center", "center", 3), new fnType("cls", "cls", 1),
			new fnType("clearscreen", "cls", 1), new fnType("cos", "cos", 2),
			new fnType("count", "count", 4), new fnType("cp", "cp", 1),
			new fnType("crossbearing", "cb", 16),
			new fnType("crosstotal", "ct", 6), new fnType("ct", "ct", 2),
			new fnType("curpos", "cp", 1), new fnType("d2r", "deg2rad", 2),
			new fnType("deg", "deg", 1), new fnType("deg2rad", "deg2rad", 2),
			new fnType("distance", "distance", 4),
			new fnType("encode", "encode", 8),
			new fnType("format", "format", 6), new fnType("goto", "goto", 6),
			new fnType("ic", "ic", 3), new fnType("ignorecase", "ic", 3),
			new fnType("instr", "instr", 12), new fnType("lcase", "lc", 2),
			new fnType("length", "len", 2), new fnType("mid", "mid", 12),
			new fnType("pc", "pz", 3), new fnType("profilecenter", "pz", 3),
			new fnType("profilecentre", "pz", 3),
			new fnType("profilzentrum", "pz", 3),
			new fnType("project", "project", 8), new fnType("pz", "pz", 3),
			new fnType("quersumme", "ct", 6), new fnType("r2d", "rad2deg", 2),
			new fnType("rad", "rad", 1), new fnType("rad2deg", "rad2deg", 2),
			new fnType("replace", "replace", 8),
			new fnType("reverse", "reverse", 2),
			new fnType("rot13", "rot13", 2), new fnType("show", "show", 2),
			new fnType("sin", "sin", 2), new fnType("skeleton", "skeleton", 3),
			new fnType("sqrt", "sqrt", 2), new fnType("sval", "sval", 2),
			new fnType("tolowercase", "lc", 2),
			new fnType("touppercase", "uc", 2), new fnType("tan", "tan", 2),
	...
	/*
	 * All errors are handled via function 'err'. Rather than creating many
	 * different Exceptions, only the standard Exception is used. err raises
	 * this exception and thereby causes the stack to be unwound until 'parse'
	 * eventually catches the exception and returns to SolverPanel, which
	 * displays the messageStack containing the error message.
	 */
	...
	private void err(String str) throws Exception {
		messageStack.add(MyLocale.getMsg(1700, "Error on line: ")
				+ thisToken.line + "  " + MyLocale.getMsg(1701, "position: ")
				+ thisToken.position);
		messageStack.add(str);
		// move cursor to error location
		if (Global.mainTab.solverP.mText.setSelectionRange(0,
				thisToken.line - 1, thisToken.position
						+ thisToken.token.length() - 1, thisToken.line - 1))
	...
	private void showVars(boolean globals) throws Exception {
		...
		while (it.hasNext()) {
			String varName = ((String) ((ewe.util.Map.MapEntry) it.next())
					.getKey());
	...
	/**
	 * Clears the symbol table of all non-global symbols (those not starting
	 * with $)
	 */
	...
	private boolean isValidCoord(String coord) {
		return CWPointFactory.getInstance().fromString(coord).isValid();
	}

	private Object getVariable(String varName) throws Exception {
		if (varName.startsWith("$")) { // Potential coordinate
			CacheHolder ch = Global.getProfile().cacheDB.get(varName
					.substring(1));
			if (ch != null) { // Found it!
		...
		Object result = symbolTable
				.get(Global.getPref().solverIgnoreCase ? varName.toUpperCase()
						: varName);
		if (result == null) {
			// If it is a global variable, add it with a default value
			if (varName.startsWith("$")) {
				result = "";
				symbolTable.put(Global.getPref().solverIgnoreCase ? varName
						.toUpperCase() : varName, "");
			} else
	...
	/**
	 * Get the top element of the calculation stack and try and convert it to a
	 * number if it is a string
	 */
	private double popCalcStackAsNumber(double defaultForEmptyString)
			throws Exception {
		double num;
		if (calcStack.get(calcStack.size() - 1) instanceof String) {
			if (((String) calcStack.get(calcStack.size() - 1)).equals(""))
				...
				num = getNumber((String) calcStack.get(calcStack.size() - 1))
						.doubleValue();
		} else {
			num = ((java.lang.Double) calcStack.get(calcStack.size() - 1))
					.doubleValue();
		}
	...
	private String popCalcStackAsString() {
		...
		if (calcStack.get(calcStack.size() - 1) instanceof Double) {
			java.lang.Double D = ((java.lang.Double) calcStack.get(calcStack
					.size() - 1));
			// Double.toString() formats numbers > 1E7 and < 1E-3 with
			// exponential notation
			// For large integers we therefore use Longs
			double d = D.doubleValue();
			// If the double is an integer and within range of longs, use Long
			if (java.lang.Math.floor(d) == d && d < java.lang.Long.MAX_VALUE
	...
	private boolean checkNextSymIs(String str) throws Exception {
		if (thisToken.token.toUpperCase().equals(str)) {
			...
			err(MyLocale.getMsg(1706, "Expected ") + str + "  "
					+ MyLocale.getMsg(1707, "Found: ") + thisToken.token);
	...
	private fnType getFunctionDefinition(String str) throws Exception {
		...
		for (int i = functions.length - 1; i >= 0; i--) {
			...
			if (functions[i].funcName.startsWith(str)) { // Partial match?
				...
				if (fnd != null)
					err(MyLocale.getMsg(1708, "Ambiguous function name: ")
							+ str);
				fnd = functions[i];
			}
		}
	...
	/**
	 * If we are in DEGree mode, convert the argument to RADiants, if not leave
	 * it unchanged
	 */
	...
	private double funcBearing() throws Exception {
		...
		double angleDeg = (CWPointFactory.getInstance().fromString(coordA))
				.getBearing(CWPointFactory.getInstance().fromString(coordB));
	...
	private void funcCenter(int nargs) throws Exception {
		if (nargs == 0) {
			...
			if (!isValidCoord(coordA))
				err(MyLocale.getMsg(1712, "Invalid coordinate: ") + coordA);
			Global.getPref().setCurCenter(
					CWPointFactory.getInstance().fromString(coordA));
	 ...
	 * or > 9223372036854775807, it is formatted using the E notation, i.e.
	 * x.xxxxxxEyy. In this case the exponent yy is also included in the
	 * crosstotal
	 */
	...
	private double funcDistance() throws Exception {
		...
		if (!isValidCoord(coordB))
			err(MyLocale.getMsg(1712, "Invalid coordinate: ") + coordB);
		double distKM = CWPointFactory.getInstance().fromString(coordA)
				.getDistance(CWPointFactory.getInstance().fromString(coordB));
	...
	/**
	 * Encode a string by replacing all characters in a string with their
	 * corresponding characters in another string
	 * 
	 * @throws Exception
	 */
	private String funcEncode() throws Exception {
		...
		if (newChars.length() != oldChars.length())
			err(MyLocale.getMsg(1711,
					"Replacement characters strings must be of equal length"));
	...
	/**
	 * Format a valid coordinate If called with one args, format the argument on
	 * the stack to CW standard The optional second argument is one of these
	 * strings "UTM","DMS","DD","DMM" or "CW"
	 * 
	 * @param nargs
	 *            1 or 2 args
	 */
	private String funcFormat(int nargs) throws Exception {
		...
		CWPoint cwPt = CWPointFactory.getInstance().fromString(coord);
		int fmt = CWPoint.CW;
		if (fmtStr.equals("dd"))
			fmt = CWPoint.DD;
		else if (fmtStr.equals("dmm"))
			fmt = CWPoint.DMM;
		else if (fmtStr.equals("dms"))
			fmt = CWPoint.DMS;
		else if (fmtStr.equals("utm"))
			fmt = CWPoint.UTM;
		else if (fmtStr.equals("gk"))
			fmt = CWPoint.GK;
		else if (!fmtStr.equals("cw"))
			err(MyLocale
					.getMsg(1713,
	...
	private void funcIgnoreVariableCase(int nargs) throws Exception {
		if (nargs == 0)
			...
			Global.getPref().solverIgnoreCase = (popCalcStackAsNumber(0) != 0) ? true
					: false;
		}
	}
	...
	private void funcPz(int nargs) throws Exception {
		if (nargs == 0) {
			...
			if (!isValidCoord(coordA))
				err(MyLocale.getMsg(1712, "Invalid coordinate: ") + coordA);
			Global.getProfile().setCenter(
					CWPointFactory.getInstance().fromString(coordA));
		}
	}

	/**
	 * Calculates the crossbearing from point1 with bearing 1 and point2 with
	 * bearing2 point1 and point 2 must be different. Not very well tested. No
	 * guarantee for correct result if any of the distance is greater than 300
	 * kilometers and / or any of the angles in the spherical triangle id
	 * greater then 90degrees
	 * 
	 * @return
	 * @throws Exception
	 */
	private String funcCrossBearing() throws Exception {
		...
		if (degrees1 < 0 || degrees1 > 360 || degrees2 < 0 || degrees2 > 360) {
			if (Global.getPref().solverDegMode) {
				err(MyLocale.getMsg(1740,
						"Crossbearing degrees must be in interval [0;360]"));
			} else {
				err(MyLocale.getMsg(1741,
						"Crossbearing degrees must be in interval [0;2*PI]"));
			}
		}
		...
		CWPoint point1 = CWPointFactory.getInstance().fromString(coordinates1);
		CWPoint point2 = CWPointFactory.getInstance().fromString(coordinates2);

		// check Parameters: bearings to project must be different from the
		// bearing between point1 and point2
	...
	private CWPoint crossbearingCalculation(CWPoint point1, CWPoint point2,
			double rAN, double rBN) throws Exception {
		// see german wikipedia keyword vorwaertsschnitt for the calculation.
		// peilung von a->b
		// Yes we will make an error, therefore we have to calculate the
		// target-point iteratively.
		// Testcode for crossbearing:
		// MP="S35 47.100 W089 43.200" # MP is centre of circle, could be any
		// waypoint
		// A=project(MP,0,1000); B=project(MP,120,1000) # Points of equilateral
		// triangle on circle
		...
		if (Math.abs(distance) <= 0.0000000001) {
			err(MyLocale.getMsg(1742,
					"Crossbearing: distance between points to small"));
		}
		...
		double bInRad = distanceInRad * java.lang.Math.sin(phi)
				/ java.lang.Math.sin(phi + psi);
		double b = bInRad * maxRadius;// * (1-flattening);
		double aInRad = distanceInRad * java.lang.Math.sin(psi)
				/ java.lang.Math.sin(phi + psi);
		...
		// if the distance between the points is to large, we will restart the
		// calculation with the new points found.
		// since the error is mostly very small these iterations are seldom used
		// and the needed depth is very low.
		// First we will make sure, that this calculation will terminate
		if (distance < errorDistance) {
			err(MyLocale
					.getMsg(1743,
							"Crossbearing calculation failed. Please inform the developers at geoclub.de"));
		}
	...
	private String funcProject() throws Exception {
		...
		if (degrees < 0 || degrees > 360)
			if (Global.getPref().solverDegMode)
				err(MyLocale.getMsg(1719,
						"Projection degrees must be in interval [0;360]"));
			else
				err(MyLocale.getMsg(1739,
						"Projection degrees must be in interval [0;2*PI]"));
		String coord = popCalcStackAsString();
		if (!isValidCoord(coord))
			err(MyLocale.getMsg(1712, "Invalid coordinate: ") + coord);
		if (Global.getPref().metricSystem == Metrics.IMPERIAL) {
		...
		return CWPointFactory.getInstance().fromString(coord).project(degrees,
				distance).toString();
	}
	...
	private void funcSkeleton(int nargs) throws Exception {
		...
		if (nStages > 0 && nStages < 30) { // e.g. sk(3)
			/*
			 * IF $01xxxx="" THEN $01xxxx="" "Station 1 = " $01xxxx
			 * goto($01xxxx); STOP ENDIF
			 */
	...
	private double funcSqrt() throws Exception {
		double a = popCalcStackAsNumber(0);
		if (a < 0)
			err(MyLocale.getMsg(1720,
					"Cannot calculate square root of a negative number"));
		return java.lang.Math.sqrt(a);
	}

	/**
	 * Replace each character by its number A=1, B=2 etc. and put result into a
	 * string
	 */
	...
	/**
	 * The following methods implement a recursive descent parser. Each method
	 * is called with 'thisToken' containing a valid token. It must return with
	 * 'thisToken' again containing a valid token.
	 */
	...
	private void parseSimpleCommand() throws Exception {
		...
		if (thisToken.token.equals("$")) { // Show all global variables
			...
			showVars(false);
			getToken();
		} else if (thisToken.tt == TokenObj.TT_VARIABLE
				&& lookAheadToken().tt == TokenObj.TT_EQ)
			parseAssign();
		else {
			parseStringExp();
	...
	private void parseIf() throws Exception {
		...
		getToken();
		// Check for "IF varName THEN" construct to check whether a variable is
		// defined
		if (thisToken.tt == TokenObj.TT_VARIABLE
				&& peekToken().token.toUpperCase().equals("THEN")) {
			...
			// numeric comparison
			try {
				Common.parseDoubleException((String) calcStack.get(calcStack
						.size() - 2));
			} catch (Exception ex) {
				compAsString = true;
			}
	...
	private void parseAssign() throws Exception {
		...
		getToken(); // =
		getToken();
		// Assigns of the format A=; are ignored so that they can stay as
		// placeholders and
		...
		if (varName.startsWith("$")) { // Potential coordinate
			...
			// Name starts with $ but is not a waypoint, fall through and set it
			// as global variable
		}
		symbolTable.put(Global.getPref().solverIgnoreCase ? varName
				.toUpperCase() : varName, popCalcStackAsString());
	}
	...
	private void parseStringExp() throws Exception {
		...
		while (thisToken.tt == TokenObj.TT_STRING
				...
				|| thisToken.tt == TokenObj.TT_SYMBOL
				&& thisToken.token.equals("(")) {
			if (thisToken.tt == TokenObj.TT_STRING) {
				calcStack.add(thisToken.token);
				getToken();
			} else {
	...
	private void parseTailExp(char unaryOp) throws Exception {
		...
		if (thisToken.tt == TokenObj.TT_FORMATSTR) {
			calcStack.add(MyLocale.formatDouble(popCalcStackAsNumber(0),
					thisToken.token).replace(',', '.'));
			getToken();
		}
	}
	...
	private void parseExpFactor() throws Exception {
		...
		if (thisToken.tt == TokenObj.TT_VARIABLE) {
			...
			err(MyLocale.getMsg(1725, "Unexpected character(s): ")
					+ thisToken.token);
		getToken();
	}
	...
	private boolean createWptIfNeeded(String wayPoint, String name,
			CacheType type) {
