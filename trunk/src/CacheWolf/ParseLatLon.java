...
/**
 * This class parses a string representation of longitude and latitude. The
 * result is a double value representing longitude and another representing
 * latitude (DD.dddd)
 */
public class ParseLatLon {
	...
	String latlon;
	/**
	 * After a calling the method parse() this variable holds the double value
	 * of latitude
	 */
	public double lat2;
	/**
	...
	public void parse() throws NumberFormatException {
		...
		try {
			...
			lat2 = Convert.parseDouble(latDeg) + Convert.parseDouble(latMin)
					/ 60.0;
			if (latNS.charAt(0) == 'S')
				lat2 = -lat2;
			lon2 = Convert.parseDouble(lonDeg) + Convert.parseDouble(lonMin)
					/ 60.0;
			if (lonEW.charAt(0) == 'W')
				lon2 = -lon2;
		} catch (Exception e) {
