 ...
 * Displays: current position,speed and bearing; relation to destination
 * waypoint<br>
 * Class ID: 1500
 */
...
public class GotoPanel extends CellPanel {
	...
	public GotoPanel(Navigate nav) {
		...
		// Button
		ButtonP.addNext(btnGPS = new mButton(MyLocale.getMsg(1504, "Start")),
		...
		ButtonP.addNext(
				...
				CellConstants.DONTSTRETCH,
				(CellConstants.DONTFILL | CellConstants.WEST));
		ButtonP.addNext(btnSave = new mButton(MyLocale.getMsg(311,
				"Create Waypoint")), CellConstants.DONTSTRETCH,
				(CellConstants.DONTFILL | CellConstants.WEST));
		ButtonP.addLast(btnMap = new mButton(MyLocale.getMsg(1506, "Map")),
				CellConstants.DONTSTRETCH,
				(CellConstants.DONTFILL | CellConstants.WEST));
		...
		mnuContextFormt.addItem(miDD = new MenuItem("d.dďż˝"));
		miDD.modifiers &= ~MenuItem.Checked;
		mnuContextFormt.addItem(miDMM = new MenuItem("dďż˝m.m\'"));
		miDMM.modifiers |= MenuItem.Checked;
		mnuContextFormt.addItem(miDMS = new MenuItem("dďż˝m\'s\""));
		...
		for (int i = 0; i < SkyOrientation.LUMINARY_NAMES.length; i++) {
			mnuContextRose.addItem(miLuminary[i] = new MenuItem(SkyOrientation
					.getLuminaryName(i)));
		...
		// Coords
		CoordsP.addNext(lblGPS = new mLabel("GPS: "),
				CellConstants.DONTSTRETCH,
				(CellConstants.DONTFILL | CellConstants.WEST));
		...
		CoordsP.addLast(lblPosition, CellConstants.HSTRETCH,
				(CellConstants.HFILL | CellConstants.WEST));

		CoordsP.addNext(lblDST = new mLabel(MyLocale.getMsg(1500, "DST:")),
				CellConstants.DONTSTRETCH,
				(CellConstants.DONTFILL | CellConstants.WEST));
		...
		CoordsP.addLast(btnGoto = new mButton(getGotoBtnText()),
		...
		mnuContextRose.addItem(miNorthCentered = new MenuItem(MyLocale.getMsg(
				1503, "North Centered")));
		...
		// add Panels
		this.addLast(ButtonP, CellConstants.DONTSTRETCH,
				CellConstants.DONTFILL | CellConstants.WEST).setTag(SPAN,
				new Dimension(2, 1));
		this.addLast(CoordsP, CellConstants.HSTRETCH,
				CellConstants.HFILL | CellConstants.NORTH).setTag(SPAN,
				new Dimension(2, 1));
		this.addLast(roseP, CellConstants.DONTSTRETCH,
				CellConstants.DONTFILL | CellConstants.WEST).setTag(SPAN,
				new Dimension(2, 1));

		// for debuging
		/*
		 * CWGPSPoint myGPS; myGPS = new CWGPSPoint(); String ex = new String();
		 * boolean test = false; try { FileReader f = new
		 * FileReader("c:\\kw\\20060928_1051.log"); ex = f.readAll(); test =
		 * myGPS.examine(ex); f.close(); } catch (IOException e ) {
		 * Vm.debug(e.toString()); } test =
		 * myGPS.examine("$GPGLL,5226.8935,N,01338.5327,E,084635.00,A,D*6E");
		 * test =
	...
	public void resizeTo(int pWidth, int pHeight) {
		...
		if (Gui.screenIs(Gui.PDA_SCREEN) && Vm.isMobile()) {
			...
			if ((Vm.getParameter(VmConstants.VM_FLAGS) & (VmConstants.VM_FLAG_SIP_BUTTON_ON_SCREEN)) == (VmConstants.VM_FLAG_SIP_BUTTON_ON_SCREEN)) {
				Rect screen = (Rect) Window.getGuiInfo(
	...
	public void updateDistance() {
		...
		if (myNavigation.gpsPos.isValid() && myNavigation.destination.isValid()) {
			distance = (float) myNavigation.gpsPos
					.getDistance(myNavigation.destination);
		}
		compassRose.setWaypointDirectionDist((float) myNavigation.gpsPos
				.getBearing(myNavigation.destination), distance);
	}
	...
	public void updateGps(int fix) {
		...
		compassRose.setGpsStatus(fix, myNavigation.gpsPos.getSats(),
				myNavigation.gpsPos.getSatsInView(), myNavigation.gpsPos
						.getHDOP());
		if ((fix > 0) && (myNavigation.gpsPos.getSats() >= 0)) {
			...
			updateDistance();
			compassRose.setSunMoveDirections((float) sunAzimut.value,
					(float) bearMov.value, (float) speed.value);
	...
	public void onEvent(Event ev) {
		if (ev instanceof MenuEvent) {
			if (ev.type == MenuEvent.SELECTED) {
				...
				if (action != null) {
					...
					lblPosition.setText(myNavigation.gpsPos
							.toString(currFormat));
					btnGoto.setText(getGotoBtnText());
				...
				if (action != null) {
					for (int i = 0; i < miLuminary.length; i++) {
						if (action == miLuminary[i]) {
							...
							miLuminary[i].modifiers |= MenuItem.Checked;
							compassRose.setLuminaryName(SkyOrientation
									.getLuminaryName(myNavigation.luminary));
						} else
							miLuminary[i].modifiers &= ~MenuItem.Checked;
					}
		...
		if (ev instanceof ControlEvent && ev.type == ControlEvent.PRESSED) {
			...
			// set current position as centre and recalculate distance of caches
			// in MainTab
			if (ev.target == btnCenter) {
				if (myNavigation.gpsPos.isValid()) {
					...
					(new MessageBox(
							...
							MyLocale
									.getMsg(1514,
											"Cannot recalculate distances, because the GPS position is not set"),
							FormBase.OKB)).execute();
			}
			// Start moving map
			if (ev.target == btnMap) {
			...
			if (ev.target == btnGoto) {
				...
				if (myNavigation.destination.isValid())
					cs.setFields(myNavigation.destination, currFormat);
				else
					cs.setFields(new CWPoint(0, 0), currFormat);
...
/**
 * class for displaying the compass rose including goto, sun and moving
 * direction
 */
class GotoRose extends AniImage {
	...
	public void doDraw(Graphics g, int options) {
		...
		roseRadius = java.lang.Math.min((location.width * 3) / 4,
				location.height) / 2;

		if (northCentered) {
			// scale(location.width, location.height, null, 0);
			// super.doDraw(g, options);
			drawFullRose(g, 0, new Color(255, 255, 255), new Color(200, 200,
					200), new Color(255, 255, 255), new Color(200, 200, 200),
					new Color(150, 150, 150), new Color(75, 75, 75), 1.0f,
					true, true);
		} else {
			int radius = (int) (roseRadius * 0.75f);

			g.setPen(new Pen(new Color(150, 150, 150), Pen.SOLID, 3));
			g.drawEllipse(location.width / 2 - radius, location.height / 2
	...
	private void drawWayPointData(Graphics g) {
		...
		if (metricSystem == Metrics.IMPERIAL) {
			...
			threshold = 0.1;
			newDistance = Metrics.convertUnit(distance, Metrics.KILOMETER,
					Metrics.MILES);
		} else {
		...
		if (newDistance >= 0.0f) {
			...
			if (tmp.value >= threshold) {
				strTemp = MyLocale.formatDouble(tmp, "0.000") + " "
						+ Metrics.getUnit(bigUnit);
			} else {
				tmp.set(Metrics.convertUnit(tmp.value, bigUnit, smallUnit));
				strTemp = tmp.toString(3, 0, 0) + " "
						+ Metrics.getUnit(smallUnit);
			}
		} else
		...
		if ((tmp.value <= 360) && (tmp.value >= -360))
			strTemp = tmp.toString(0, 0, 0) + " "
					+ MyLocale.getMsg(1502, "deg");
		else
	...
	private void drawGpsData(Graphics g) {
		...
		if (Global.getPref().metricSystem == Metrics.IMPERIAL) {
			tmp.set(Metrics.convertUnit(m_speed, Metrics.KILOMETER,
					Metrics.MILES));
			unit = " mph";
			strSpeed = "- mph";
		} else {
		...
		if ((tmp.value <= 360) && (tmp.value >= -360))
			strMoveDir = tmp.toString(0, 0, 0) + " "
					+ MyLocale.getMsg(1502, "deg");

		int textWidth = java.lang.Math.max(fm.getTextWidth(strSpeed), fm
				.getTextWidth(strMoveDir));
	...
	private void drawLuminaryData(Graphics g) {
		...
		if (sunDir < 360 && sunDir > -360) {
			Double tmp = new Double();
			tmp.set(sunDir);
			strSunDir = tmp.toString(0, 0, 0) + " "
					+ MyLocale.getMsg(1502, "deg");
		}

		int textWidth = java.lang.Math.max(fm.getTextWidth(m_Luminary), fm
				.getTextWidth(strSunDir));
	...
	private void drawGpsStatus(Graphics g) {
		...
		String strSats = "Sats: -";
		if (m_sats >= 0) {
			strSats = "Sats: " + Convert.toString(m_sats) + "/"
					+ Convert.toString(m_satsInView);
		}
		String strHdop = "HDOP: -";
		if (m_hdop >= 0)
			strHdop = "HDOP: " + Convert.toString(m_hdop);

		int textWidth = java.lang.Math.max(fm.getTextWidth(strSats), fm
				.getTextWidth(strHdop));
		int startX = location.width - (textWidth + 4);
		int startY = location.height - 2 * lineHeight;
		g.fillRect(startX, startY, location.width - startX, location.height
				- startY);
	...
	private void drawArrows(Graphics g) {
		if (g != null) {
			...
			if (gotoDir < 360 && gotoDir > -360 && moveDir < 360
					&& moveDir > -360) {
			...
			if (northCentered) {
				...
				// moveDir centered
				if (moveDir < 360 && moveDir > -360) {
					// drawDoubleArrow(g, 360 - moveDir, BLUE, new
					// Color(175,0,0), 1.0f);
					// drawRose(g, 360 - moveDir, new Color(100,100,100), new
					// Color(200,200,200), 1.0f);
					drawFullRose(g, 360 - moveDir, new Color(255, 255, 255),
							new Color(200, 200, 200), new Color(150, 150, 150),
							new Color(200, 200, 200), new Color(200, 200, 200),
							new Color(75, 75, 75), 1.0f, false, false);

					int radius = (int) (roseRadius * 0.75f);
					g.setPen(new Pen(RED, Pen.SOLID, 3));
					g.drawLine(location.width / 2,
							location.height / 2 - radius, location.width / 2,
							location.height / 2 + radius);

					if (gotoDir < 360 && gotoDir > -360)
						drawThinArrow(g, gotoDir - moveDir, Color.DarkBlue,
								moveDirColor, 1.0f);
					if (sunDir < 360 && sunDir > -360)
	...
	private void drawSunArrow(Graphics g, float angle, Color col, float scale) {
		...
		int circleX = centerX
				+ new Float(circlePos * java.lang.Math.sin(angleRad))
						.intValue();
		int circleY = centerY
				- new Float(circlePos * java.lang.Math.cos(angleRad))
						.intValue();
		...
		pointsX[0] = centerX
				+ new Float(arrowLength * java.lang.Math.sin(angleRad))
						.intValue();
		pointsY[0] = centerY
				- new Float(arrowLength * java.lang.Math.cos(angleRad))
						.intValue();
		pointsX[1] = centerX
				+ new Float(halfArrowWidth
						* java.lang.Math
								.sin(angleRad + java.lang.Math.PI / 2.0))
						.intValue();
		pointsY[1] = centerY
				- new Float(halfArrowWidth
						* java.lang.Math
								.cos(angleRad + java.lang.Math.PI / 2.0))
						.intValue();
		pointsX[2] = centerX
				+ new Float(halfArrowWidth
						* java.lang.Math.sin(angleRad + java.lang.Math.PI))
						.intValue();
		pointsY[2] = centerY
				- new Float(halfArrowWidth
						* java.lang.Math.cos(angleRad + java.lang.Math.PI))
						.intValue();
		pointsX[3] = centerX
				+ new Float(halfArrowWidth
						* java.lang.Math
		...
		g.fillEllipse(circleX - circleRadius, circleY - circleRadius,
				2 * circleRadius, 2 * circleRadius);
	}

	private void drawThinArrow(Graphics g, float angle, Color col,
			Color colPoint, float scale) {
		float angleRad = (angle) * (float) java.lang.Math.PI / 180;
		int centerX = location.width / 2, centerY = location.height / 2;
		float arrowLength = roseRadius * scale;
		...
		pointsX[0] = centerX
				+ new Float(sideLineLength
						* java.lang.Math.sin(angleRad - halfOpeningAngle))
						.intValue();
		pointsY[0] = centerY
				- new Float(sideLineLength
						* java.lang.Math.cos(angleRad - halfOpeningAngle))
						.intValue();
		pointsX[1] = centerX
				+ new Float(arrowLength * java.lang.Math.sin(angleRad))
						.intValue();
	...
	private void drawFullRose(Graphics g, float angle, Color colLeft,
			Color colRight, Color colNorthLeft, Color colNorthRight,
			Color colBorder, Color colText, float scale, boolean bDrawText,
		...
		if (bDrawEightArrows) {
			innerScale = 0.12f;
			drawRosePart(g, 45 + angle, colLeft, colRight, colBorder, colText,
					scale * subScale2, innerScale, "NE", bDrawText);
			drawRosePart(g, 135 + angle, colLeft, colRight, colBorder, colText,
					scale * subScale2, innerScale, "SE", bDrawText);
			drawRosePart(g, 225 + angle, colLeft, colRight, colBorder, colText,
					scale * subScale2, innerScale, "SW", bDrawText);
			drawRosePart(g, 315 + angle, colLeft, colRight, colBorder, colText,
					scale * subScale2, innerScale, "NW", bDrawText);
		}

		drawRosePart(g, 0 + angle, colNorthLeft, colNorthRight, colBorder,
				colText, scale * subScale1, innerScale, "N", bDrawText);
		drawRosePart(g, 90 + angle, colLeft, colRight, colBorder, colText,
				scale * subScale1, innerScale, "E", bDrawText);
		drawRosePart(g, 180 + angle, colLeft, colRight, colBorder, colText,
				scale * subScale1, innerScale, "S", bDrawText);
		drawRosePart(g, 270 + angle, colLeft, colRight, colBorder, colText,
				scale * subScale1, innerScale, "W", bDrawText);
	}

	private void drawRosePart(Graphics g, float angle, Color colLeft,
			Color colRight, Color colBorder, Color colText, float scale,
			float innerScale, String strDir, boolean bDrawText) {
		float angleRad = angle * (float) java.lang.Math.PI / 180;
		...
		pointsX[2] = centerX
				+ new Float(halfArrowWidth
						* java.lang.Math
								.sin(angleRad - java.lang.Math.PI / 4.0))
						.intValue();
		pointsY[2] = centerY
				- new Float(halfArrowWidth
						* java.lang.Math
								.cos(angleRad - java.lang.Math.PI / 4.0))
						.intValue();
		...
		if (bDrawText) {
			...
			Font tempFont = new Font(mainFont.getName(), Font.BOLD,
					tempFontSize);
			g.setFont(tempFont);
			...
			float stringGap = (float) java.lang.Math.sqrt(stringHeight
					* stringHeight + stringWidth * stringWidth);

			float stringPosition = arrowLength - stringGap / 2.0f;
			g.setColor(colText);
			g.drawText(strDir, centerX
					+ new Float(stringPosition
							* java.lang.Math.sin(angleRadText) - stringWidth
							/ 2.0f).intValue(), centerY
					- new Float(stringPosition
							* java.lang.Math.cos(angleRadText) + stringHeight
							/ 2.0f).intValue());
