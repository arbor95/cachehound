...
/**
 * Class to caculate positions of luminaries all methods are static usage: call
 * utc2juliandate and then getLuminaryDir in ressources/cachewolf.languages
 * messege numbers from 6100
 * 
 * @author Pfeffer
 * 
 */
public class SkyOrientation {
	...
	public static String[] LUMINARY_NAMES = { MyLocale.getMsg(6100, "Sun"),
	...
	public static String[] LUMINARY_DESC = { MyLocale.getMsg(6100, "Sun"),
			MyLocale.getMsg(6101, "Moon"),
	...
	/**
	 * get azimuth from north and elevation for horizont for a given Luminary
	 * (planet or star)
	 * 
	 * @param luminary
	 *            one of SUN, MOON, ALIOTH, GRAETER_BEAR, ALNILAM, ORION,
	 *            CASSIOPEIA_GAMMA, CASSIOPEIA
	 * @param jd
	 ...
	 */
	public static CWPoint getLuminaryDir(int luminary, double jd,
			CWPoint onEarth) {
		switch (luminary) {
		case SUN:
		...
		default:
			return equatorial2AzimutCoos(onEarth, jd,
					STARS[luminary - MOON - 1]);
		}
	}
	...
	public static double utc2juliandate(String utc, String datum) {
		try {
			...
			double jd = java.lang.Math.floor(365.25 * (jahr + 4716.))
					...
					+ (double) stunde / 24 + (double) minute / 1440
					+ (double) sekunde / 86400 + b - 1524.5;
			return jd;
			// double jd0 = java.lang.Math.floor(365.25*(jahr + 4716.)) +
			// java.lang.Math.floor(30.6001*((double)monat+1.)) +(double)tag + b
			// - 1524.5;
		} catch (IndexOutOfBoundsException e) {
			// wird von substring geworfen wenn datum / utc nicht genug Ziffern
			// haben
			// NumberFormatException wird außerdem von Convert.ParseInt direkt
			// geworfen wenn
	 ...
	 * @return
	 */
	public static float getSunAzimut(String utc, String datum, double lat,
			double lon) {
		// (new MessageBox("test",
		// "utc:"+utc+" datum: "+datum+", lat: "+lat+", len: "+lon,
		// MessageBox.OKB)).exec();
		try {
			int tag, monat, jahr, stunde, minute, sekunde;
			tag = Convert.parseInt(datum.substring(0, 2));
			...
			double jd = java.lang.Math.floor(365.25 * (jahr + 4716.))
					...
					+ (double) sekunde / 86400 + b - 1524.5;
			double jd0 = java.lang.Math.floor(365.25 * (jahr + 4716.))
					+ java.lang.Math.floor(30.6001 * (monat + 1.)) + tag + b
					- 1524.5;
			// Ekliptikalkoordinaten der Sonne berechnen (see
			// http://de.wikipedia.org/wiki/Sonnenstand )
			double n = jd - 2451545.0;
			double l = 280.46 + 0.9856474 * n;
			double g = 357.528 + 0.9856003 * n;
			double d = l + 1.915
					* java.lang.Math.sin(g / 180 * java.lang.Math.PI) + 0.02
					* java.lang.Math.sin(2 * g / 180 * java.lang.Math.PI);
			// Rektaszension alpha und Deklination delta der Sonne berechnen
			double e = 23.439 - 0.0000004 * n;
			double alphaNenner = java.lang.Math
					.cos(d / 180 * java.lang.Math.PI);
			double alpha = 180
					/ java.lang.Math.PI
					* java.lang.Math.atan(java.lang.Math.cos(e / 180
							* java.lang.Math.PI)
							* java.lang.Math.sin(d / 180 * java.lang.Math.PI)
							/ alphaNenner);
			double delta = 180
					/ java.lang.Math.PI
					* java.lang.Math.asin(java.lang.Math.sin(e / 180
							* java.lang.Math.PI)
							* java.lang.Math.sin(d / 180 * java.lang.Math.PI));
			if (alphaNenner < 0) {
				alpha += 180;
			...
			double azimutNenner = java.lang.Math.cos((theta - alpha) / 180
					* java.lang.Math.PI)
					* java.lang.Math.sin(lat / 180 * java.lang.Math.PI)
					- java.lang.Math.tan(delta / 180 * java.lang.Math.PI)
					* java.lang.Math.cos(lat / 180 * java.lang.Math.PI);
			float azimut = (float) java.lang.Math.atan(java.lang.Math
					.sin((theta - alpha) / 180 * java.lang.Math.PI)
					/ azimutNenner);
			...
			// ewe.sys.Vm.debug("sun Elevation: " +getSunAzimut2 (utc, datum,
			// lat, lon).latDec);
			// CWPoint MoonDir = getMoonDir(jd, new CWPoint(lat, lon));
			// ewe.sys.Vm.debug("Moon Elevation: " + MoonDir.latDec +
			// "Moon Azimut: " + MoonDir.lonDec);
			// CWPoint OrionDir = getAlnilamDir(jd, new CWPoint(lat, lon));
			// ewe.sys.Vm.debug("Alnilam (Orion) Elevation: " + OrionDir.latDec
			// + "Alnilam (Orion) Azimut: " + OrionDir.lonDec );

			return azimut;
		} catch (IndexOutOfBoundsException e) {
			// wird von substring geworfen wenn datum / utc nicht genug Ziffern
			// haben
			// NumberFormatException wird ausserdem von Convert.ParseInt direkt
			// geworfen wenn
			// nicht in Int konvertiert werden kann
			throw new NumberFormatException();
		}
	...
	public static CWPoint getSunAzimut2(String utc, String datum, double lat,
			double lon) {
	...
	public static CWPoint getAlnilamDir(double jd, CWPoint onEarth) {
		// Koordinaten Alnilam (mittlerer Guertelstern des Orion), Rektaszension
		// 5h36m13s; Deklination -1°12'7 TODO Aequinoktium 2000
		// Source: wikipedia
	...
	// ignores the time difference between juliandate and TDT, which is
	...
	public static CWPoint getMoonEclipticCoos(double julianDate) {
		...
		double sunAnomalyMean = 360 * DEG / 365.242191
				* (julianDate - 2447891.5) + 279.403303 * DEG - 282.768422
				* DEG;
		...
		moonCoor.lonDec = ((N2 + Math.atan2(Math.sin(l3 - N2) * Math.cos(i),
				Math.cos(l3 - N2))) * RAD) % 360;
	...
	public static CWPoint ecliptic2AzimutCoos(CWPoint onEarth,
			double julianDate, CWPoint ecliptic) {
	 ...
	 */
	public static CWPoint equatorial2AzimutCoos(CWPoint onEarth,
			double julianDate, CWPoint equatorial) {
		...
		double azimutNenner = Math.cos(tau) * Math.sin(phi)
				- Math.tan(equatorial.latDec / 180 * Math.PI)
				* Math.cos(onEarth.latDec / 180 * java.lang.Math.PI);
		float azimut = (float) java.lang.Math.atan(java.lang.Math
				.sin((theta - equatorial.lonDec) / 180 * Math.PI)
				/ azimutNenner);
		azimut = (float) (azimut * 180f / java.lang.Math.PI);
		if (azimutNenner < 0)
			azimut += 180.;
		double h = 180
				/ Math.PI
				* Math.asin(Math.cos(equatorial.latDec / 180 * Math.PI)
						* Math.cos(tau) * Math.cos(phi)
						+ Math.sin(equatorial.latDec / 180 * Math.PI)
						* Math.sin(phi));
		// null = Sueden auf Null = Norden umrechnen
		azimut += 180.;
		if (azimut > 360.)
	...
	public static CWPoint ecliptic2Equatorial(CWPoint eklipCoo,
		...
		equatorial.latDec = 180
				...
				* Math.asin(Math.sin(eklipCoo.latDec / 180 * Math.PI) * coseps
						+ Math.cos(eklipCoo.latDec / 180 * Math.PI) * sineps
						* sinlon); // deklination (delta)
