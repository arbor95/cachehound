...
/**
 * class to handle a list of maps it loads the list, finds the best map for a
 * given location, says if a map is available for a given lat lon at a given
 * scale start offset for language file: 4700
 * 
 */
public class MapsList extends Vector {
			...
			if (!dirs.get(j).equals(".")) {
				...
				if (dirstmp != null) {
					for (int subDir = 0; subDir < dirstmp.length; subDir++) {
						dirs.add(j + 1 + subDir, dirs.get(j) + "/"
								+ dirstmp[subDir]);
					}
				}
			}
	...
	/*
	 * diese Routine wird gegenwärtig für 3 ZWecke verwendet: a) normal - keep
	 * given resolution --> Lösung: übergebene scale nutzen für screen b)
	 * highest res: Ziel: Karte mit höchster Auflösung, die im screen ist und
	 * möglichst nah an lat/lon -> ich muss auflösung noch in Dateinamen
	 * schreiben c) gegenteil von b)
	 */
	/**
		...
		for (int i = size() - 1; i >= 0; i--) {
			...
			if (screenArea.isOverlapping(mi)) { // is on screen
				if (!forceScale || (forceScale && !scaleEquals(scale, mi))) { // different
					...
					if (!forceScale
						...
						if (bestMap == null
								|| scaleNearerOrEuqal(mi.scale, bestMap.scale,
										scale)) {
							latNearer = java.lang.Math.abs(ll.latDec
									- mi.center.latDec)
									/ mi.sizeKm < minDistLat;
							lonNearer = java.lang.Math.abs(ll.lonDec
									- mi.center.lonDec)
					...
					if (better) {
						minDistLat = java.lang.Math.abs(ll.latDec
								- mi.center.latDec)
								/ mi.sizeKm;
						minDistLon = java.lang.Math.abs(ll.lonDec
								- mi.center.lonDec)
								/ mi.sizeKm;
						bestMap = mi;
						// Vm.debug("better"+ i);
	...
	/*
	 * public MapInfoObject getBestMapNotStrictSciale(double lat, double lon,
	 * Area screen, float scale) { MapInfoObject ret = getBestMap(lat, lon,
	 * screen, scale, true); if (ret == null) ret = getBestMap(lat, lon, screen,
	 * scale, false); return ret; }
	 */
	/**
	...
	public MapInfoObject getMapForArea(CWPoint topleft, CWPoint bottomright) {
		...
		MapInfoObject mi;
		String cmp = "FF1"
				+ (new Area(topleft, bottomright)).getEasyFindString();
		...
		for (int i = size() - 1; i >= 0; i--) {
			...
			if (mi.isInBound(topleft) && mi.isInBound(bottomright)) { // both
				...
				if (fittingmap == null
					...
					if (scaleEquals(mi, fittingmap)) { // same scale as bestmap
						...
						latNearer = java.lang.Math.abs(topleft.latDec
								- mi.center.latDec)
								/ mi.sizeKm < minDistLat;
						lonNearer = java.lang.Math.abs(topleft.lonDec
								- mi.center.lonDec)
								/ mi.sizeKm < minDistLon;
						if (latNearer && lonNearer)
				...
				if (better) {
					fittingmap = mi;
					minDistLat = java.lang.Math.abs(topleft.latDec
							- mi.center.latDec);
					minDistLon = java.lang.Math.abs(topleft.lonDec
							- mi.center.lonDec);
				}
			}
		} // for
	 ...
	 * @param moreDetails
	 *            true: find map with more details == higher resolustion = lower
	 *            scale / false find map with less details = better overview
	 * @return
	 */
	public MapInfoObject getMapChangeResolution(CWPoint ll, Rect screen,
			float curScale, boolean moreDetails) {
		if (size() == 0)
			return null;
		long start = new Time().getTime();
		InfoBox progressBox = null;
			...
			if (screenArea.isOverlapping(mi)) { // is on screen
				if (bestMap == null || !scaleEquals(mi, bestMap)) { // different
					...
					latNearer = java.lang.Math
							...
							/ mi.sizeKm < minDistLat;
					lonNearer = java.lang.Math
							.abs(ll.lonDec - mi.center.lonDec)
							/ mi.sizeKm < minDistLon;
					if (latNearer && lonNearer)
	 ...
	 * @param lon
	 * @param scale
	 *            scale (meters per pixel) of the map for which the screen edges
	 *            are wanted
	 * @param map
	 *            map for which the screen edges are wanted
	 * @return
	 */
	private Area getAreaForScreen(Rect a, CWPoint ll, float scale,
			MapInfoObject map) {
		Area ret = null;
		Point xy = map.calcMapXY(ll);
		Point topleft = new Point(xy.x - a.x, xy.y - a.y);
		ret = new Area(map.calcLatLon(topleft), map.calcLatLon(topleft.x
				+ a.width, topleft.y + a.height));
		return ret;
	}
	 ...
	 * @param old
	 * @param wanted
	 * @return true if test is nearer to wanted than old, false if the change in
	 *         the scale is lower than scaleTolerance
	 */
	 ...
	 * isInAmap(CWPoint topleft, CWPoint buttomright) { if
	 * (!latRangeList.isInRange(topleft.latDec) ||
	 * !latRangeList.isInRange(buttomright.latDec)) ||
	 * !lonRangeList.inInRange(topleft.lonDec) ||
	 * !lonRangeList.isInRange(buttomright.lonDec) return false; }
	 */
}
...
class MapListEntry /* implements Comparable */{
		...
		if (sortEntryBBox == null) { // || sortEntryScaleCenterPx.length() < 16)
			...
			try {
				...
				if (rename == 0) { // never asked before
					if ((new MessageBox(
							MyLocale.getMsg(4702, "Optimisation"),
							MyLocale
									.getMsg(
											4703,
						...
						renameProgressInfoB = new InfoBox(MyLocale.getMsg(327,
								"Info"), MyLocale.getMsg(4704,
								"\nRenaming file:")
								+ "    \n");
				...
				if (rename == 1) {
					...
					if (f != null) {
						...
						if (!new File(f).rename(to)) {
							Global.getPref().log(
									"MapListEntry (String pathi, String filenamei): Failed to rename: "
											+ path + filename + ": " + f
											+ ".wfl to: " + to);
							(new MessageBox(
						...
						Global
								.getPref()
								.log(
										"MapListEntry (String pathi, String filenamei): Could not find image assiciated to: "
												+ path + filename + ".wfl");
						(new MessageBox(
								MyLocale.getMsg(321, "Error"),
								MyLocale
										.getMsg(4709,
												"Could not find image assiciated to:\n")
										+ path + filename + ".wfl",
								FormBase.OKB)).exec();
					}
				...
				(new MessageBox(MyLocale.getMsg(321, "Error"), MyLocale.getMsg(
						...
						FormBase.OKB)).exec();
				Global.getPref().log(
						"MapListEntry (String pathi, String filenamei): I/O-Error while reading: "
								+ path + filename + ": ", ioex);
			} catch (Exception ex) {
				(new MessageBox(MyLocale.getMsg(321, "Error"), MyLocale.getMsg(
						4706, "Error while reading:")
						+ " " + path + filename + ": " + ex.getMessage(),
						FormBase.OKB)).exec();
				Global.getPref().log(
						"MapListEntry (String pathi, String filenamei): Error while reading: "
								+ path + filename + ": ", ex);
			}
		}
	}
	...
	/*
	 * // this maybe needed some time later to further enhance the speed of
	 * finding the best map public int compareTo(Object other) { if (other ==
	 * null) return 1; return
	 * this.sortEntryBBox.compareTo(((MapListEntry)other).sortEntryBBox); }
	 */
}
