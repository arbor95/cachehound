...
public class TrackOverlay extends MapImage {
	...
	public TrackOverlay(TrackPoint topLefti, int widthi, int highti,
			MapInfoObject transi) {
		super();
		topLeft = new TrackPoint(topLefti);
		trans = transi;
		pixelShift = trans.calcMapXY(topLeft);
		bottomRight = trans.calcLatLon(widthi + pixelShift.x, highti
				+ pixelShift.y);
		...
		// int[] markMaskOpaque = {0xffffffff, 0xffffffff, 0xffffffff,
		// 0xffffffff};
		// mask.setPixels( markMaskOpaque, 0, 50, 50, 2, 2, 0);
		// draw.fillRectRGB(50, 50, 52, 52, 0x00ff0000); // fillRectRGB has a
		// Bug - it never returns - use fillRect instead
		// image.setPixels(markImage, 0, 50, 50, 2, 2, 0); // out of an to me
		// unkwon reason this doesn't work here, but it does in painttracks
	}
	...
	// ==================================================================
	{
		...
		if (image != null && image != sourceImage)
			image.freeze();
		// if (mask != null && mask != sourceMask) mask.freeze(); // dont freeze
		// the mask, it could change. Anyway momentanously it doesnt change,
		// because when the image contains non-white in the opaque areas, it
		// will be opaque without changing the mask
	...
	public void paintTracks() {
		...
		// draw.fillRect(50, 50, 4, 4); // fillRectRGB has a Bug - it never
		// returns - use fillRect instead
		...
		if (numberOfTracks > 1) {
			numberOfPoints += (numberOfTracks - 1)
					* ((Track) tracks.get(0)).num;
		}
		int n = 0;
	...
	public boolean paintPoint(Color f, TrackPoint where) {
		if (where.latDec < bottomRight.latDec || where.latDec > topLeft.latDec
		...
		/*
		 * if (image.bufferedImage != null) { // funktioniert gut, allerdings
		 * nur in der java-VM wenn ewe.fx.Image.bufferedImage als public
		 * definiert int yd; for (int xd=-1; xd<=1; xd++) { for (yd=-1; yd<=1;
		 * yd++) { ((BufferedImage)image.bufferedImage).setRGB(x+xd, y+yd,
		 * -65536); } }
		 */
	...
	/**
	 * this method forces ewe to transfer the drawn points from _awtImage to
	 * bufferedImage, which is drawn to the screen
	 * 
	 */
	private void fixate() {
		if (numPixels == 0)
	...
	public void addPixel(int x, int y, Color f)
	...
	public void paintLastAddedPoint(Track tr) {
		// draw.setPen(new Pen((Color) tr.trackColor,Pen.SOLID,3));
		draw.setColor(tr.trackColor);
		if (paintPoint(tr.trackColor, tr.TrackPoints[tr.num - 1]))
...
/*
 * draw zeichnet auf _awtImage image.drawImage erzeugt bufferedImage, wenn es
 * vorher null war und kopiert den entsprechenden Teil in das übergebene Surface
 * Das Problem ist, dass bufferedImage nicht mehr upgedatet wird, wenn es einmal
 * erzugt wurde. Es wird nur dann upgedatet, wenn das Image ein anderes wird
 * oder die Maske eine andere. Das Update erfolgt dann über doDraw, was
 * doCheckMask aufruft, das das eigentliche kopieren aus _awtImage vornimmt.
 * 
 * Problem: das kopieren von _awtImage in BufferedImage ist sehr zeitaufwändig,
 * weil es keine native Routine ist und jedes Pixel einzeln geprüft wird.
 * Deswegen wäre die beste Lösung, wenn ich bufferedImage direkt updaten könnte.
 * Aber bufferedImage ist privat, ich kann auch in abgeleiteten Klassen nicht
 * drauf zugreifen.
 * 
 * Zur Not wäre auch denkbar, doDraw zu überschreiben, um bei jedem Aufruf alle
 * Trackpoints neu zu zeichnen. Work-Aorund:
 * draw.drawImage(image,null,Color.Pink,0,0,1,1); bewirkt, dass awtImage ins
 * bufferedImage kopiert wird. Dabei wird die transparentColor (in mImage) nicht
 * geändert und beim Aufruf von doDraw wird wieder die ursprüngliche
 * transparentColor verwendet
 */
...
// image
// ((BufferedImage)(image).se.^.bufferedImage.raster.data[y*this.location.width
// + x]=-65536; //was dort steht wird tatsächlich angezeigt, allerdings kann ich
// es nicht direkt setzen :-(
// int[] markPixels = new int[4];
// for (int i = 0; i<markPixels.length; i++) { markPixels[i] = -65536; }
// image.transparent = null; hilft auhc nicht
// image.mask = null;
// image.bufferedImage = null;
// image.setPixels(markPixels, 0 , x-20, y, 2, 2, 0); // dadrin sollte
// bufferedImage = null gesetzt werden, wird es aber nicht :-(
// ewe.fx.mImage mark = new mImage();
// Image mark = new Image(2,2);
// new Graphics(mark).drawImage(image, null, transparentColor, x-40, y, 2, 2);
// mark.draw(draw, x-50, y, Graphics.DRAW_OVER); // options (Graphics.DRAW_OVER)
// are ignored anyway
 ...
 * transparenten Farbe nicht (Hintergrund wird weiß statt durchsichtig) deswegen
 * (und weil in ewe-VM effizienter) Umstellung auf Transparenzmaske statt
 * transparenter Farbe TODO Dies ist in Java-VM allerdings extrem Speicher
 * fressend -> evtl abfragen static int fixMask(WObject image,WObject col,int
 * isMask): in Maske: 0 an durchsichtiger Stelle, sonst ff in Image: ffffff an
 * durchsichtiger Stelle
 * 
 * in java-VM in Maske: ffffffff in image.mask, wenn nicht durchsichtig ff000000
 * an durchsichtiger Stelle image.doCheckMask erzeugt ein Image mit 0 an den
 * durchsichtigen Stellen, die dadurch definiert sind, dass im image 0xffffff
 * und in (mask & 0xffffff == 0) steht.
 */
/*
 * this class is only needed to have a fast access to the list of pixels which
 * are added but aniimage.draw will not lead to a change on the screen so that
 * these pixels will be drawn seperately by doDraw
 */
