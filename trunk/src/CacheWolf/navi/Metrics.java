...
public final class Metrics {
	 ...
	 *            Value to convert
	 * @param sourceUnit
	 *            Constant from class Metrics to represent unit in which
	 *            <code>value</code> is meant.
	 * @param targetUnit
	 *            Constant from class Metrics to represent the unit in which
	 *            <code>value</code> has to be transformed
	 * @return The new converted value
	 * @throws UnsupportedOperationException
	 *             when the conversion between the two units is not programmed.
	 */
	public static final double convertUnit(double value, int sourceUnit,
			int targetUnit) {
		/*
		 * Strategy: For each metrical system we define a standard unit: km and
		 * mi. In a first step any input value is converted to the standard unit
		 * in its system Then (if required) the standard units are converted
		 * between the systems. Last step: The converted unit is converted to
		 * the target unit (which is in the same metrical system).
		 */
		...
		if (sourceUnit == targetUnit) {
			...
			default:
				Global.getPref().log(
						"Cannot convert unit" + getUnit(sourceUnit));
				throw new UnsupportedOperationException("Cannot convert unit"
						+ getUnit(sourceUnit));
			}
			...
			default:
				Global.getPref().log(
						"Cannot convert unit" + getUnit(targetUnit));
				throw new UnsupportedOperationException("Cannot convert unit"
						+ getUnit(targetUnit));
			}
		}
	...
	/**
	 * Returns the constant for the metric systems standard unit (km or mi.) a
	 * given unit belongs to.
	 * 
	 * @param unit
	 *            Unit to examine
