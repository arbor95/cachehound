...
/**
 * This class is for importing and manually georeferencing maps This class
 * id=4100 for cachewolf-languages
 */
public class MapImporter extends Form {
	...
	/**
	 * When a user clicks on the map and more than three ground control points
	 * exist then the calculated coordinate based on the affine transformation
	 * is displayed in the info panel below the map. It helps to identify how
	 * good the georeferencing works based on the set GCPs.
	 */
	public void updatePosition(int px, int py) {
		if (GCPs.size() >= 3 || (wfl.topleft.isValid())) {
			CWPoint p = wfl.calcLatLon(px, py);
			infLabel
					.setText("--> " + p.getLatDeg(CWPoint.DMS) + " "
							+ p.getLatMin(CWPoint.DMM) + " / "
							+ p.getLonDeg(CWPoint.DMS) + " "
							+ p.getLonMin(CWPoint.DMM));
		}
	}
	 ...
	 */
	public MapImporter(Preferences pref, String mapToLoad,
			boolean worldfileexists) {
		this.pref = pref;
		...
		this.addLast(scp.getScrollablePanel(), CellConstants.STRETCH,
				CellConstants.FILL);
		...
		infPanel.addLast(infButton, CellConstants.DONTSTRETCH,
				CellConstants.FILL);
	...
	/**
	 * Add a ground control point to the list If the list is longer than 3 GCPs
	 * these will be evaluated to obtain the required parameters for the affine
	 * transformation.
	 */
	public void addGCP(GCPoint GCP) {
		if (GCP.latDec > 90 || GCP.latDec < -90 || GCP.lonDec > 360
				|| GCP.lonDec < -180)
	...
	/**
	 * Method to copy ("import") a png based map into the maps folder in the
	 * CacheWolf base directory.
	 * 
	 * If the maps directory does not exist it will create it. If it finds .map
	 * files it will assume these are oziexplorer calibration files. It will use
	 * these files to automatically georeference the files during import.
	 */
	public int importMap() {
		String rawFileName = new String();
		FileChooser fc = new FileChooser(FileChooserBase.DIRECTORY_SELECT,
				Global.getPref().getBaseDir().getAbsolutePath());
		...
		File mapFile;
		InfoBox inf = new InfoBox("Info", MyLocale.getMsg(4109,
				"Loading maps...            \n"),
		...
		int len;
		String[] parts;
		String[] files = inDir.listMultiple("*.png,*.jpg,*.gif,*.bmp",
				FileBase.LIST_FILES_ONLY);
		...
		for (int i = num - 1; i >= 0; i--) {
			...
			try {
				...
				imageerror = true;
				inf.addWarning("\n"
						+ MyLocale.getMsg(4112,
								"IO-Error while copying image from: ")
						+ curInFullPath + MyLocale.getMsg(4113, " to: ")
				...
				inf.addWarning("\n"
						...
						+ curInFullPath
						+ MyLocale.getMsg(4116, " - image not copied"));
			} finally {
				try {
					if (in != null)
						in.close();
			...
			if (!imageerror && mapFile.exists()) {
				...
				try {
					...
					while ((line = inMap.readLine()) != null) {
						if (line.equals("MMPNUM,4")) {
							...
							gcpG = new GCPoint(Convert.toDouble(parts[3]),
							...
							addGCP(gcpG);
							/*
							 * already read from image file itself // get
							 * dimensions of image while ( (line =
							 * inMap.readLine()) != null){ if
							 * (line.startsWith("IWH")){ parts =
							 * mString.split(line, ','); imageWidth =
							 * Convert.toInt(parts[2]); imageHeight =
							 * Convert.toInt(parts[3]); } }
							 */
							wfl.evalGCP(GCPs, imageWidth, imageHeight);
							// Vm.debug("Saving .map file to: " + mapsPath + "/"
							// + rawFileName + ".wfl");
				...
				if (!imageerror)
					inf.addWarning("\n"
							+ MyLocale.getMsg(4119,
									"No calibration file found for: ")
							+ currfile + " - you can calibrate it manually");
			}
		} // for file
	...
	/**
	 * Handles button pressed event When the button is pressed a mapname.wfl
	 * file is saved in the maps directory.
	 */
				...
				while (retry == true) {
					try {
						...
						MessageBox tmpMB = new MessageBox(MyLocale.getMsg(321,
								...
								"Error writing file ")
								+ e.getMessage()
								+ MyLocale.getMsg(324, " - retry?"),
								FormBase.YESB | FormBase.NOB);
						if (tmpMB.execute() == FormBase.IDYES)
							retry = true;
					} catch (IllegalArgumentException e) {
						MessageBox tmpMB = new MessageBox(MyLocale.getMsg(144,
								"Warning"), MyLocale.getMsg(325,
								"Map not calibrated")
								+ MyLocale.getMsg(324, " - retry?"),
								FormBase.YESB | FormBase.NOB);
						if (tmpMB.execute() == FormBase.IDYES) {
...
/**
 * Class that creates a panel and loads a map. It catches click events to
 * display a form where the user may enter the required ccordinates The data is
 * stored as a ground control point in the calling class: Map
 */
