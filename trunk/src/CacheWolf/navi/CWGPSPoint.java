...
/**
 * @author Kalle Class for decoding NMEA sentences
 */
...
public class CWGPSPoint extends CWPoint implements TimerProc {
	...
	/**
	 * this method should be called, if not data is coming from COM-Port but is
	 * expected to come
	 */
	...
	/**
	 * this method should be called, if examine returns for several calls that
	 * it couldn't interprete the data
	 */
	...
	public int startLog(String logFileDir, int seconds, int flag) {
		...
		String logFileName = new String(logFileDir + currTime.toString()
				+ ".log");
		// create Logfile
		try {
	...
	public boolean examine(String NMEA) {
		...
		try {
			 ...
			 */while (true) {
				...
				if ((end - start) < 15
						|| !checkSumOK(NMEA.substring(start, end + 3))) {
				...
				Extractor ex = new Extractor("," + NMEA.substring(start, end),
						",", ",", 0, true);
				...
				if (currToken.equals("$GPGGA")) {
					...
					while (ex.endOfSearch() != true) {
						...
						case 2:
							...
							try {
								latMin = currToken.substring(2, currToken
										.length());
								interpreted = true;
							} catch (IndexOutOfBoundsException e) {
								latlonerror = true;
						...
						case 4:
							...
							try {
								lonMin = currToken.substring(3, currToken
										.length());
								interpreted = true;
							} catch (IndexOutOfBoundsException e) {
								latlonerror = true;
						...
						case 8:
							try {
								...
								Global.getPref().log("Ignored Exception", e,
										true);
							}
							break;
						case 9:
					...
					if (Fix > 0)
						this.set(latNS, latDeg, latMin, "0", lonEW, lonDeg,
								lonMin, "0", CWPoint.DMM);
						...
						case 7:
							if (status.equals("A")) {
								try {
									...
									Global.getPref().log("Ignored Exception",
											e, true);
								}
							}
							break;
	 ...
	 * 
	 * @param gps
	 *            GPSD string with data to examine Format: GPSD,key=value,...
	 * @return true if some data could be interpreted false otherwise
	 */
	public boolean examineGpsd(String gps) {
		...
		while (!ex.endOfSearch()) {
			...
			if (part.startsWith("A=") && part.indexOf('?') < 0) {
				...
				// Returns the current position in the form "P=%f %f"; numbers
				// are in degrees, latitude first.
				if (part.indexOf('?') < 0) {
					this.Fix = 1;
				...
				// Returns "Q=%d %f %f %f %f %f": a count of satellites used in
				// the last fix,
	...
	private boolean checkSumOK(String nmea) {
		...
		// Vm.debug(nmea.substring(3,6)+" Checksum: " + nmea.substring(endPos+1)
		// + " Calculated: " + Convert.intToHexString(checkSum));
		try {
