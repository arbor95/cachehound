 ...
 * allows the user to click on an image that will then be displayed in its
 * original size as long as the image fits the application size. If the
 * application size is not sufficient then the image will be scaled to the
 * available screen size. A right mouseclick on an image will open a dialogue to
 * delete the file.
 */
public class ImagePanel extends InteractivePanel {
	...
	public void setImages(CacheHolderDetail cache) {
		if (cache != oldCache) {
			...
			this.virtualSize = r;
			// this.setPreferredSize(pref.myAppWidth,
			// rowCounter*thumb_size+rowCounter*padding+40);
			this.checkScrolls();
			this.refresh();
			locY = 0;
	...
	private void addImages(CacheImages pImages) {
		...
		for (int i = 0; i < pImages.size(); i++) {
			...
			try {
				...
				// actuall new mImage(location); should do the following "if"
				// but it doesn't anyhow
	...
	/**
	 * React to when a user clicks an image. If left mouse key is clicked, will
	 * open a new window displaying the image scaled to window size if the image
	 * is larger, otherwise the true size is displayed. If right mouse key is
	...
	public void imageClicked(AniImage which, Point pos) {
		...
		if ((ControlBase.currentPenEvent.modifiers & PenEvent.RIGHT_BUTTON) == PenEvent.RIGHT_BUTTON
			...
			if (which instanceof ImagePanelImage
					&& !((ImagePanelImage) which).fileName.equals(NO_IMAGE)) {
				MessageBox mBox = new MessageBox(MyLocale
						.getMsg(144, "Warning"), MyLocale.getMsg(344,
						"Delete image")
						+ " \"" + ((ImagePanelImage) which).imageText + "\"?",
			...
			if (which instanceof ImagePanelImage) {
				...
				try {
					...
					(new MessageBox(
							...
							MyLocale
									.getMsg(343,
											"Not enough free memory to load cache image")
	...
	/**
	 * Create a "pen held down" event on hardware that does not support a right
	 * mouse key (e.g. Windows Mobile) by measuring the time between pen down
	 * and pen up events. This is used in imageClicked to differentiate between
