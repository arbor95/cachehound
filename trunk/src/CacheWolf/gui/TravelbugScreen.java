...
public class TravelbugScreen extends Form {
	...
	public TravelbugScreen(TravelbugList tbl, String title, boolean allowNew) {
		...
		this.addLast(sbp);
		this.addNext(btCancel = new mButton(MyLocale.getMsg(614, "Cancel")),
				CellConstants.DONTSTRETCH, CellConstants.FILL);
		this.addLast(btAccept = new mButton("OK"), CellConstants.DONTSTRETCH,
				CellConstants.FILL);
	...
	private class myList extends SimpleList {
		...
		public String getDisplayItem(int idx) {
			if (idx == size)
				...
				// If the name contains HTML entities, we need to convert it
				// back
