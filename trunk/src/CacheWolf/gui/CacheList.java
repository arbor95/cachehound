 ...
 * by dragging them out or selecting them and pressing the "delete" key. Within
 * the list the selected cache can be moved up/down using two buttons. The
 * finished list can be saved and reloaded with the selected position being
...
public class CacheList extends CellPanel {
	...
	public CacheList() {
		...
		// Button to toggle whether additional waypoints are automatically
		// dragged
		// with the parent waypoint
		addLast(chkAddAddis = new mCheckBox(MyLocale.getMsg(193, "add Addis")),
				HSTRETCH, HFILL);
		chkAddAddis.setToolTip(MyLocale.getMsg(186, "Also drag Addi Wpts"));
		// Finally button to apply the list as a filter
		addLast(btnFilter = new mButton(MyLocale.getMsg(189, "Apply List")),
				HSTRETCH, HFILL);
	...
	private mButton btnDown, btnUp, btnLoad, btnNew, btnSave, btnSaveAs,
			btnFilter;
	private mImage imgOpen, imgNew, imgSave, imgSaveAs;
	/**
	 * This list mirrors the items in the list of selected caches for faster
	 * access. When the list of selected caches is manipulated (btnUp, btnDown),
	 * this list is also kept up to date
	 */
	...
	private class myList extends mList {
		...
		public void onKeyEvent(KeyEvent ev) {
			/*
			 * This is a bit of a hack. By default Ewe sends key events to this
			 * panel. So if the list has not had anything dragged into it, we
			 * redirect the focus to the list view, assuming that that is where
			 * the key event needs to go.
			 */
			if (needsInit && ev.target == this) {
				Gui.takeFocus(Global.mainTab.tbP.tc,
	...
	private void changeUpDownButtonStatus() {
		...
		if (needsInit || lstCaches.itemsSize() < 2
				|| lstCaches.getSelectedIndex(0) == 0)
			btnUp.modify(Disabled, 0);
		btnDown.modify(0, Disabled);
		if (needsInit || lstCaches.itemsSize() < 2
				|| lstCaches.getSelectedIndex(0) == lstCaches.itemsSize() - 1)
			btnDown.modify(Disabled, 0);
		btnUp.repaintNow();
		btnDown.repaintNow();
	...
	public void onEvent(Event ev) {
		if (ev instanceof MenuEvent && ev.type == MenuEvent.SELECTED) {
			if (lstCaches.itemsSize() > 0 && !needsInit) {
				...
				// Ensure that the main view is updated with the selected cache,
				// i.e.
				...
				if (activeTab == 0) {
					...
					// We need to change to the list view first to load a new
					// cache
					Global.mainTab.onEvent(new MultiPanelEvent(0,
							Global.mainTab, 0));
					Global.mainTab.tbP.selectRow(idx);
					Global.mainTab.onEvent(new MultiPanelEvent(0,
							Global.mainTab, activeTab));
				}
			}
		}
	...
	public void applyCacheList() {
		...
		if (wrongVisStatus > 0)
			(new MessageBox(
					...
					MyLocale
							.getMsg(4600,
									"Some cache(s) cannot be shown because of wrong blacklist status"),
	...
	private boolean addCache(CacheHolder ch) {
		if (cacheList.getIndex(ch.getWayPoint()) < 0) {
			...
			lstCaches.addItem((new MenuItem()).iconize(ch.getWayPoint() + "   "
					+ ch.getCacheName(), GuiImageBroker.getInstance()
					.getTypeImage(ch.getType()), true));
			dirty = true;
			return true;
		} else
			return false;
	...
	private void updateScreen() {
		...
		if ((selPanel = Global.mainTab.cardPanel.selectedItem) > -1) {
			if (selPanel == 1) {
				// postEvent(new
				// MultiPanelEvent(MultiPanelEvent.SELECTED,Global.mainTab,1));
	...
	public void saveIfDirty() {
		if (dirty) {
			if ((new MessageBox(MyLocale.getMsg(144, "Warning"), MyLocale
					.getMsg(192, "Save changes"), FormBase.MBYESNO)).execute() == FormBase.IDYES) {
				if (currFile != null)
					saveToFile(currFile);
				else {
	...
	private void saveToFile(String fileName) {
		...
		try {
			...
			for (int i = 0; i < cacheList.size(); i++) {
				...
				outp.print((i == selectedIndex ? ">" : "")
						+ (cacheList.get(i)).getWayPoint() + "\n");
			}
			outp.close();
		} catch (Exception e) {
	...
	private void setTitle(String fileName) {
		...
		if (localFileName.lastIndexOf('/') > 0)
			localFileName = localFileName.substring(localFileName
					.lastIndexOf('/') + 1);
		// Drop the extension
		if (localFileName.indexOf('.') > 0)
			lblTitle.setText(localFileName.substring(0, localFileName
					.indexOf('.')));
		else
