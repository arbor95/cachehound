...
public class TravelbugJourneyScreen extends Form {
	...
	private mInput inpName, inpTrackingNo, inpFromDate, inpFromProfile,
			inpFromWaypoint, inpToDate, inpToProfile, inpToWaypoint;
	...
	public TravelbugJourneyScreen() {
		...
		if (curCacheNo >= 0 && curCacheNo < cacheDB.size()) {
			...
			tblSrcCache = ch.getCacheDetails(false, true).getTravelbugs();
		}
		title = "Travelbugs" + cache;
		tcTbJourneyList = new tbListControl();
		tcTbJourneyList
				.setTableModel(modTbJourneyList = new tbListTableModel());
		tablepane.addLast(new MyScrollBarPanel(tcTbJourneyList,
				ScrollablePanel.AlwaysShowVerticalScrollers), STRETCH, FILL);
		...
		pnlName.addNext(new mLabel(MyLocale.getMsg(6025, "Name:")),
				DONTSTRETCH, DONTFILL);
		pnlName.addLast(inpName = new mInput(), HSTRETCH, HFILL);
		pnlName.addNext(new mLabel(MyLocale.getMsg(6026, "Tracking #:")),
				DONTSTRETCH, DONTFILL);
		pnlName.addLast(inpTrackingNo = new mInput(), HSTRETCH, HFILL);
		pnlName.addNext(new mLabel(MyLocale.getMsg(6027, "ID/GUID:")),
				DONTSTRETCH, DONTFILL);
		...
		pnlFrom.addNext(new mLabel(MyLocale.getMsg(6029, "Profile/Cache:")),
		...
		pnlFrom.addNext(new mLabel(MyLocale.getMsg(6030, "Date found:")),
				DONTSTRETCH, DONTFILL | WEST);
		pnlFrom.addNext(inpFromDate = new mInput(), CellConstants.HSTRETCH,
				(CellConstants.HFILL | CellConstants.WEST));
		pnlFrom.addLast(btnFromDate = new mButton(new mImage("calendar.png")),
				DONTSTRETCH, DONTFILL | WEST);
		btnFromDate.modify(0, ControlConstants.TakesKeyFocus);

		pnlFrom.addNext(new mLabel(MyLocale.getMsg(6031, "Logged:")),
				DONTSTRETCH, DONTFILL | WEST);
		pnlFrom.addLast(chkFromLogged = new mCheckBox(""), DONTSTRETCH,
				DONTFILL | WEST);
		...
		pnlTo.addNext(new mLabel(MyLocale.getMsg(6029, "Profile/Cache:")),
				DONTSTRETCH, DONTFILL | WEST);
		pnlTo.addNext(inpToProfile = new mInput(), HSTRETCH, HFILL);
		pnlTo.addLast(inpToWaypoint = new mInput(), HSTRETCH, HFILL);

		pnlTo.addNext(new mLabel(MyLocale.getMsg(6033, "Date dropped:")),
				DONTSTRETCH, DONTFILL | WEST);
		pnlTo.addNext(inpToDate = new mInput(), CellConstants.HSTRETCH,
				(CellConstants.HFILL | CellConstants.WEST));
		// inpToDate.modifyAll(DisplayOnly,0);
		pnlTo.addLast(btnToDate = new mButton(new mImage("calendar.png")),
				DONTSTRETCH, DONTFILL | WEST);
		btnToDate.modify(0, ControlConstants.TakesKeyFocus);
		// pnlTo.addLast(new mLabel(""));

		pnlTo.addNext(new mLabel(MyLocale.getMsg(6031, "Logged:")),
				DONTSTRETCH, DONTFILL | WEST);
		pnlTo.addLast(chkToLogged = new mCheckBox(""), DONTSTRETCH, DONTFILL
				| WEST);
		...
		modTbJourneyList.columnMap = TableColumnChooser.str2Array(Global
				.getPref().travelbugColMap, 0, 11, 0, -1);
		modTbJourneyList.colWidth = TableColumnChooser.str2Array(Global
				.getPref().travelbugColWidth, 10, 1024, 50, -1);
	...
	public void onEvent(Event ev) {
		...
		if (ev instanceof ControlEvent && ev.type == ControlEvent.PRESSED
			...
			if (ev.target == btnFromDate || ev.target == btnToDate) {
				mInput inpDate = ev.target == btnFromDate ? inpFromDate
						: inpToDate;
				...
				if (dc.execute() == ewe.ui.FormBase.IDOK) {
					inpDate
							.setText(Convert.toString(dc.year) + "-"
									+ MyLocale.formatLong(dc.month, "00") + "-"
									+ MyLocale.formatLong(dc.day, "00") + " "
									+ dc.time);
					if (ev.target == btnFromDate) {
						tblMyTravelbugJourneys.getTBJourney(selectedRow)
								.setFromDate(inpDate.getText());
						Gui.takeFocus(chkFromLogged,
								ControlConstants.ByKeyboard);
					} else {
						tblMyTravelbugJourneys.getTBJourney(selectedRow)
								.setToDate(inpDate.getText());
						Gui.takeFocus(chkToLogged, ControlConstants.ByKeyboard);
					}
					tcTbJourneyList.repaint();
				}
	...
	class tbListTableModel extends TableModel {
		...
		public Object getCellData(int row, int col) {
			if (row == -1) {
				...
				if (col == 0
						&& (!tblMyTravelbugJourneys.getTBJourney(row)
								.getFromLogged() || !tblMyTravelbugJourneys
								.getTBJourney(row).getToLogged())) {
					// Is it a column with a checkbox?
					if (map != 7 && map != 11)
						return new IconAndText((IImage) imgRed,
								(String) tblMyTravelbugJourneys.getTBJourney(
										row).getElementByNumber(map), fm);
					else { // Checkbox - special treatment
						IconAndText iat = new IconAndText(imgRed, "", fm);
						iat.addColumn(tblMyTravelbugJourneys.getTBJourney(row)
								.getElementByNumber(map));
						return iat;
					}
				} else
					return tblMyTravelbugJourneys.getTBJourney(row)
							.getElementByNumber(map);
			}
		}
		...
		public TableCellAttributes getCellAttributes(int row, int col,
			...
			if (row >= 0)
				...
				case 6: // fromDate
					if (!tblMyTravelbugJourneys.getTBJourney(row)
							.getFromLogged())
						ta.foreground = RED;
					break;
				case 10: // toDate
		...
		public boolean penPressed(Point onTable, Point cell) {
			...
			if (cell != null && cell.y == -1) { // Hit a header => sort the
				...
				if ((tcTbJourneyList.mnuToggleList.modifiers & MenuItem.Checked) == MenuItem.Checked) {
					tblMyTravelbugJourneys.sortFirstHalf(columnMap[cell.x],
							sortAsc, modTbJourneyList.numRows);
		...
		private void toggleSelect(int fromRow, int toRow) {
			...
			tcTbJourneyList.addToSelection(new Rect(0, fromRow, numCols, toRow
					- fromRow + 1), false, true);
		}
	...
	class tbListControl extends TableControl {
		private MenuItem mnuNewTB, mnuDeleteTB, mnuGetMission, mnuOpenOnline,
				mnuDropTB, mnuPickupTB, mnuDeleteTBs;
		public MenuItem mnuToggleList;
		private Menu mnuFullMenu, mnuDeleteMenu;

		tbListControl() {
			MenuItem[] TBMenuItems = new MenuItem[10];
			TBMenuItems[0] = mnuPickupTB = new MenuItem(MyLocale.getMsg(6040,
					"Pick up TB from current cache"));
			TBMenuItems[1] = mnuDropTB = new MenuItem(MyLocale.getMsg(6041,
					"Drop TB in cache"));
			TBMenuItems[2] = new MenuItem("-");
			TBMenuItems[3] = mnuNewTB = new MenuItem(MyLocale.getMsg(6042,
					"New Travelbug"));
			TBMenuItems[4] = mnuDeleteTB = new MenuItem(MyLocale.getMsg(6043,
					"Delete Travelbug"));
			TBMenuItems[5] = new MenuItem("-");
			TBMenuItems[6] = mnuGetMission = new MenuItem(MyLocale.getMsg(6044,
					"Get Mission"));
			TBMenuItems[7] = mnuOpenOnline = new MenuItem(MyLocale.getMsg(6045,
					"Open on-line"));
			TBMenuItems[8] = new MenuItem("-");
			TBMenuItems[9] = mnuToggleList = new MenuItem(MyLocale.getMsg(6046,
					"Show only not logged"));
			...
			TBMenuItemsDel[0] = mnuDeleteTBs = new MenuItem(MyLocale.getMsg(
		...
		public void popupMenuEvent(Object selectedItem) {
			if (selectedItem == mnuPickupTB) {
				...
				if (tb != null) {
					chDmodified = true;
					tblMyTravelbugJourneys.addTbPickup(tb,
			...
			if (selectedItem == mnuDropTB) {
				if (selectedRow >= 0 && selectedRow < modTbJourneyList.numRows) {
					Travelbug tb = tblMyTravelbugJourneys.getTBJourney(
							selectedRow).getTb();
			...
			if (selectedItem == mnuGetMission && selectedRow > -1) {
				...
				if (tbj.getTb().getGuid().length() != 0) {
					tbj.getTb().setMission(
							spider.getBugMissionByGuid(tbj.getTb().getGuid()));
				} else {
					// try to get mission and name by tracking number
					boolean suceeded = false;
					if (tbj.getTb().getTrackingNo().length() != 0) {
						suceeded = spider.getBugMissionAndNameByTrackNr(tbj
								.getTb());
					}
					// if this has't worked, try to get ID by name
					if (!suceeded) {
						tbj.getTb().setGuid(
								spider.getBugId(tbj.getTb().getName().trim()));
						// if we have an ID now, get mission by ID
						if (tbj.getTb().getGuid().length() != 0) {
							tbj.getTb().setMission(
									spider.getBugMissionByGuid(tbj.getTb()
											.getGuid()));
						}
					}
				}
				...
				if (tbj.getTb().getGuid().length() != 0) {
					...
					try {
						...
						if (tbj.getTb().getGuid().length() > 10)
							s = "http://www.geocaching.com/track/details.aspx?guid="
									+ tbj.getTb().getGuid();
						else
		...
		public void toggleNonLogged() {
			...
			if ((mnuToggleList.modifiers & MenuItem.Checked) == MenuItem.Checked) {
				...
				modTbJourneyList.numRows = tblMyTravelbugJourneys
						.countNonLogged();
			} else {
				modTbJourneyList.numRows = tblMyTravelbugJourneys.size();
			}
		...
		public void cursorTo(int row, int col, boolean selectNew) {
			...
			selectedRow = row;
			if (row >= 0) {
				modTbJourneyList.showFields(tblMyTravelbugJourneys
						.getTBJourney(row));
			} else {
				modTbJourneyList.showFields(new TravelbugJourney(""));
			}
