...
import CacheWolf.util.Parser;
import CacheWolf.util.Tokenizer;
import ewe.fx.Color;
...
public class SolverPanel extends CellPanel {
	...
	public mTextPad mText;
	public OutputPanel mOutput;
	Preferences pref;
	Profile profile;
	String currFile;
	...
	/**
	 * Sets the instructions in the solver panel. The last remembered cache of
	 * the solver panel is not changed.
	 * 
	 * @param text
	...
	/**
	 * Sets the instructions of the solver panel to the solver code in the given
	 * cache. The current cache is remembered. If the instructions are set for
	 * an other cache, then the output panel is cleared if the cache objects do
	...
	public void setInstructions(CacheHolder pCh) {
		if (pCh != null) {
			this.setInstructions(pCh.getCacheDetails(true, false).getSolver());
	...
	private class OutputPanel extends mTextPad {
		...
		OutputPanel() {
			...
			setMenu(mnuContext = getClipboardMenu(new Menu(
					new MenuItem[] { mnuClr = new MenuItem(MyLocale.getMsg(
							1734, "Clear output")) }, "")));
		}
	...
	public SolverPanel(Preferences p, Profile prof) {
		...
		programPanel.addLast(new MyScrollBarPanel(mText = new InputPanel()))
				.setTag(SPAN, new Dimension(2, 1));
		CellPanel pnlStatButtons = new CellPanel();
		pnlStatButtons.addNext(btnDegRad = new mButton(getSolverDegMode()),
		...
		pnlButtons.addNext(mBtSolve = new mButton(MyLocale.getMsg(1735,
				"Solve!")), CellConstants.HSTRETCH, CellConstants.HFILL);
		pnlButtons.addLast(btnWolfLang = new mButton(MyLocale.getMsg(118,
				"WolfLanguage")), CellConstants.HSTRETCH, CellConstants.HFILL);
		pnlButtons.equalWidths = true;
		pnlStatButtons.addLast(pnlButtons, CellConstants.HSTRETCH,
				CellConstants.HFILL);
		programPanel.addLast(pnlStatButtons, HSTRETCH, HFILL);
		/*
		 * programPanel.addNext(btnLoad= new
		 * mButton(MyLocale.getMsg(1736,"Load")),CellConstants.DONTSTRETCH,
		 * (CellConstants.HFILL|CellConstants.WEST));
		 * programPanel.addNext(btnSave= new
		 * mButton(MyLocale.getMsg(1737,"Save")),CellConstants.DONTSTRETCH,
		 * (CellConstants.HFILL|CellConstants.WEST));
		 * programPanel.addLast(btnSaveAs= new
		 * mButton(MyLocale.getMsg(1738,"SaveAs")),CellConstants.DONTSTRETCH,
		 * (CellConstants.HFILL|CellConstants.WEST));
		 */
	...
	public void onEvent(Event ev) {
		if (ev instanceof DataChangeEvent)
			Global.mainTab.cacheDirty = true;
		if (ev instanceof ControlEvent && ev.type == ControlEvent.PRESSED) {
			...
			/*
			 * if(ev.target == btnLoad){ FileChooser fc = new
			 * FileChooser(FileChooser.OPEN, profile.dataDir);
			 * 
