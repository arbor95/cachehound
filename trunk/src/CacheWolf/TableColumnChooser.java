...
public class TableColumnChooser extends CellPanel {
	 ...
	 * @param selectedCols
	 *            The selected columns separated by ,
	 */
	public TableColumnChooser(String[] colNames, String selectedCols) {
		...
		addNext(new MyScrollBarPanel(lstShown = new myList(6, shownCols),
				ScrollablePanel.AlwaysShowVerticalScrollers));
		...
		addLast(new MyScrollBarPanel(lstHidden = new myList(6, hiddenCols),
				ScrollablePanel.AlwaysShowVerticalScrollers));
	...
	private void changeUpDownButtonStatus() {
		...
		if (lstShown.items.size() < 2
				|| lstShown.getSelectedIndex(0) == lstShown.items.size() - 1)
			btnDown.modify(Disabled, 0);
		btnUp.repaintNow();
		btnDown.repaintNow();
	}
	...
	private class myList extends mList {
		...
		public void startDragging(DragContext dc) {
			idx = getSelectedIndex(0);
			// Can only drag if we have a valid index (at least on element in
			// list)
			// Also if we drag from lstShown, we must leave at least one item in
			// list
			if (idx >= 0 && idx < items.size()
					&& (this != lstShown || items.size() > 1)) {
	...
	/**
	 * Converts a comma delimited string into an integer array. Each value is
	 * checked and has to be between min and max, If not it is replaced with
	 * default
	 * 
	 * @param minSize
	 *            TODO
	 */
	public static int[] str2Array(String configString, int min, int max,
			int def, int minSize) {
