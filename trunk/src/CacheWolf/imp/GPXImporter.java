...
public class GPXImporter extends MinML {
	...
	public void endElement(String name) {
		...
		if (name.equals("wpt")) {
			...
			if (index == -1) {
				holder.setNoFindLogs(holder.getFreshDetails().CacheLogs
						.countNotFoundLogs());
				holder.setNew(true);
				cacheDB.add(holder);
					...
					if (spiderOK == true && holder.is_archived() == false) {
						...
						if (holder.getWayPoint().startsWith("GC") || fromTC) {
							...
							while (ex.endOfSearch() == false
								...
								if (num >= holder.getFreshDetails().images
										.size())
									break;
								imgName = holder.getFreshDetails().images.get(
										num).getTitle();
								holder.getFreshDetails().LongDescription = replace(
